...
class DriverBase {
	...
	get info () {
		return  {
			...
			name : this?._name,
			modbusAllowed: this.modbusAllowed,
...
class InverterSun2000 extends DriverBase{
	...
	get standby() {
		...
		const newStandby = offlineStates.includes(state?.value);
		if (newStandby != this._standby) {
			if (newStandby) {
				this.adapter.log.info(`The device ${this.deviceInfo.index} switches to standby mode.`);
			} else {
				this.adapter.log.info(`The device ${this.deviceInfo.index} switches from standby to normal mode.`);
			}
		}
