...
class DriverBase {
	...
	async updateStates(modbusClient,refreshRate,duration) {
		...
		//if (await this._isStandby()) return 0;
		await this._isStandby();

		const start = this._now.getTime();
		//this.inverterInfo = inverter; constructor
		...
		for (const reg of this.registerFields) {
			...
			//brauch nicht mehr per Inverter zu sein!
...
class DriverInfo extends DriverBase {
	constructor (stateInstance,inverter,options) {
		super(stateInstance,inverter,{
			name: 'Huawei DriverInfo',
			...options,
		});
		this._detectedModelId = 0;

		//this.inverterInfo.modelId = 0;
		...
		const newFields = [
			{
				...
				postHook: (path) => {
					const modelId = this.stateCache.get(path+'info.modelID')?.value;
					if (modelId) {
						this._detectedModelId = modelId;
					} else {
						this._detectedModelId = -1;
					}
				}
			}
		];
	...
	get info () {
		return  {
			...
			name : this?._name,
			stanby: this?._standby,
			detectedModelId : this._detectedModelId
		};
	}
}

class DriverSun2000 extends DriverBase{
	constructor(stateInstance,inverter,options) {
		super(stateInstance,inverter,
			{
				name: 'sun2000',
		...
		const newHooks = [
			{
				...
				fn : (path) => {
					...
					let inputYield = this.stateCache.get(path+'dailyEnergyYield')?.value + charge*0.97 - disCharge;
					// #### TEST ####
					/*
	...
	async _isStandby() {
		//Test value 0xA000
		const standbyStates = [0xA000,0x0000,0x0001,0x0002,0x0003,0x0100,0x0300,0x0301,0x0302,0x0303,0x0304,0x0305,0x0306,0x0307,0x0308];
		const state = this.stateCache.get(this.inverterInfo.path+'.deviceStatus');
		if (this._standby) {
			const wakeUp = 30*60*1000; //wake up
			if ((this._now?.getTime() - this.adapter.settings.sunrise?.getTime()) <  wakeUp|| !standbyStates.includes(state?.value )) {
				this._standby = false;
				this.adapter.log.info('inverter '+this.inverterInfo.index+' switch to normal operating mode.');
			}
		} else {
			if ( standbyStates.includes(state?.value) ) {
				this._standby = true ;
				this.adapter.log.info('inverter '+this.inverterInfo.index+' switch to standby mode.');
			}
		}
		return this._standby;
	}
...
class DriverSun2000_M1 extends DriverSun2000{
	constructor(stateInstance,inverter,options) {
		super(stateInstance,inverter,{
...
function getDriverHandler(modelId) {
	...
	if (modelId == 0) return DriverInfo;
	if (model_sun2000M1.includes(modelId)) return DriverSun2000_M1;
	return DriverSun2000;
}
