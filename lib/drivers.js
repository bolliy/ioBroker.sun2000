...
class DriverSun2000 extends DriverBase{
	...
	//if the modbus-device online we cannot read or write anythink!
	_CheckIfOnline() {
		//Test value= 0xA000
		this._newNow();
		...
		//device offline?
		if (!this._online) {
			const wakeUp = 30*60*1000; //wake up until 30 Minutes after sunrise
			if ((this._now?.getTime() - this.adapter.settings.sunrise?.getTime()) <  wakeUp|| !offlineStates.includes(state?.value )) {
				newOnline = true;
				this.adapter.log.info(`The Modbus interface of Inverter ${this.inverterInfo.index} switches to online mode.`);
			}
		} else {
			if (state && offlineStates.includes(state?.value) ) {
				newOnline = false ;
				this.adapter.log.info(`The Modbus interface of Inverter ${this.inverterInfo.index} switches to offline mode.`);
			}
		}
		return newOnline;
	}
