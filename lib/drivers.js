...
class DriverBase {
	...
	get info () {
		return  { modelId : this?._modelId, name : this?._name };
	}
			...
			for(const field of reg.states) {
				...
				if (field.register) {
					...
					if (value !== null) {
						...
						if (reg.address == 32064) {
							console.log('+++++ '+reg.address+' '+reg.info+' data:'+data+' value:'+value+' +++++');
							console.log(path+state.id);
						}
	...
	async updateStates(modbusClient,refreshRate,duration) {
		...
		for (const reg of this.registerFields) {
			...
			try {
				...
				if (reg.address == 32064) {
					console.log('+++++ '+reg.address+' '+reg.info+' data:'+data+' +++++');
				}
		...
		const newFields = [
			{
				...
				postHook: (path) => {
					...
					if (modelId) {
						this.inverterInfo.modelId = modelId;
					} else {
						this.inverterInfo.modelId = -1;
					}
				}
			}
		];
