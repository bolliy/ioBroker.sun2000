...
class ModbusServer {
	...
	_waitForInitialized() {
		return new Promise(resolve => {
			...
			// Fallback: Falls Event bereits gefeuert oder nie kommt
			const timeout = this.adapter.setTimeout(() => {
				this.log.warn('Modbus-proxy: initialized event timeout, assuming ready');
				done();
			}, 2000);

			this.serverTCP.once('initialized', () => {
	...
	async _handleGetReg(fnName, startAddr, length, unitId, callback) {
		...
		try {
			...
			if (device) {
				...
				const isMissing = values[0] == null;
				const isStale = typeof device.isRegisterStale === 'function' && device.isRegisterStale(startAddr, length);

				if ((isMissing || isStale) && this.adapter.modbusClient) {
					try {
						this.log.debug(
							`Modbus-proxy: ${isMissing ? 'cache miss' : 'stale cache'} for id/address/length ${unitId}/${startAddr}/${length} - reading directly from device`,
						);
						this.adapter.modbusClient.setID(device.modbusId);
						const data = await this.adapter.modbusClient.readHoldingRegisters(startAddr, length, this.log);
						device.addHoldingRegisters(startAddr, data); //refresh cache + timestamp for subsequent requests
						this._addInfoStat(`${fnName}-direct`, startAddr, length, unitId);
						await this.wait(50);
						callback(undefined, data);
						return;
					} catch (err) {
						if (isMissing) {
							//no fallback data available at all - must report an error
							this.log.warn(`Modbus-proxy: direct read failed for id/address/length ${unitId}/${startAddr}/${length}: ${err?.message}`);
							this._addInfoStat(`#${fnName}-direct`, startAddr, length, unitId);
							await this.wait(200);
							callback({ modbusErrorCode: err?.modbusCode ?? 0x04, msg: err?.message || 'Slave device failure (data not yet available)' });
							return;
						}
						//stale but direct read failed - better to serve the (old) cached value than to fail outright
						this.log.debug(
							`Modbus-proxy: direct read failed for id/address/length ${unitId}/${startAddr}/${length}, serving stale cache instead: ${err?.message}`,
						);
						this._addInfoStat(`#${fnName}-direct-fallback`, startAddr, length, unitId);
					}
				}
