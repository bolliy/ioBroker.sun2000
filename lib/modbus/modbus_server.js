...
class ModbusServer {
	...
	getDeviceInstance(unitId) {
		...
		for (const device of this.adapter.devices) {
			if (device.instance) {
				//for TestMode
				if (realUnitId === 0 && device.instance?._name === 'emma') {
					this.log.info(`Modbus-proxy: check device ${device.instance._name} with id ${device.instance.modbusId} for unitId ${realUnitId}`);
	...
	async _handleGetReg(fnName, startAddr, length, unitId, callback) {
		...
		try {
			...
			if (device) {
				...
				//this.adapter.log.debug(`### modbus-proxy - _handleGetReg: Device Info ${JSON.stringify(device?.info)}`);
				//device.logHoldingRegisters(startAddr, length); //for TestMode
