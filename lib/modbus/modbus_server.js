...
class ModbusServer {
		...
		return new Promise((resolve) => {
			if (this.serverTCP) {
				this.serverTCP.on('initialized', () => {
					this.log.info('ModbusTCP server initialized');
	...
	async connect() {
		try {
			...
			if (inUse) {
				this.log.error('TCP Port '+this._port+' is now in use!');
			} else {
				this.log.info('ModbusTCP listening on modbus://'+this._ip+':'+this._port);
				await this.close();
				...
				this.serverTCP.on('serverError',async (err) => {
					this.log.error('ModbusTCP server (serverError) '+err);
					this.serverTCP && await this.close();
				});
				this.serverTCP.on('socketError', async (err) => {
					//Network error
					if (err == 'ECONNRESET' ) {
						this.log.warn('ModbusTCP server (socketError) '+err);
						this.serverTCP && await this.close();
					} else {
						this.log.error('ModbusTCP server (socketError) '+err);
					}
				});
			}
		} catch (err){
			this.log.error('ModbusTCP server couldnt listen '+err?.message);
		}
	}
		...
		return new Promise((resolve) => {
			...
			if (this.serverTCP) {
				this.serverTCP.close(() => {
					this.log.info('ModbusTCP server closed');
					resolve({});
				});
			} else {
				resolve({});
