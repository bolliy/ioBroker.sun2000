	...
	getDeviceInstance(unitId) {
		for (const device of this.adapter.devices) {
			if (device.instance) {
				//for TestMode
				/*
				this.log.info(`### Modbus-proxy: check device ${device.instance._name} with id ${device.instance.modbusId} for unitId ${unitId}`);
				if (unitId === 0 && device.instance?._name == 'emma') {
		...
		try {
			...
			if (device) {
				...
				if (this.adapter.settings.ms.log) {
					this.log.info(`Modbus-proxy: read data from device/id/address/data ${device._name}/${unitId}/${startAddr}/${values}`);
				}
