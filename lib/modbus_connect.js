...
class ModbusConnect extends DeviceInterface {
	constructor(adapterInstance,options) {
		...
		this._id = 0;
		if (options.modbusDelay === undefined) options.modbusDelay = 20; //ms
		//nullish coalescing Operator (??)
		this._to = {
			timeout : options?.modbusTimeout ?? 5000,
			delay : options?.modbusDelay ?? 50,
			startDelay : options?.modbusStartDelay ?? 2000,
			min : options.modbusDelay,
			max : 2000,
			lastAccessTime : 0,
			successCounter : 0,
			errorCounter : 0,
			modbusIdChangedDelay : options.modbusDelay*2,
		};
	}
	...
	async _checkError(err) {
		...
		if (err.modbusCode == null) {
			await this.close();
			this.adapter.log.debug('modbus socked closed');
	...
	async connect(repeatCounter = 0) {
		try {
			...
			await this.delay(this._to.startDelay);
