	...
	async _checkError(err) {
		...
		if (err.modbusCode === undefined) {
			if (err.errno == 'ETIMEDOUT' ) {
				this.adapter.log.warn('Modbus socket was closed, has another device interrupted the connection?');
				this._adjustDelay(false); //increase delay
				if (this._to.delay < 5000) {
					this.adapter.log.warn('If it doesn\'t apply, increase the delay value.');
				}
			}
			await this.close();
			await this._create();
