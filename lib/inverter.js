...
class InverterInstance {
	constructor(stateInstance,inverter) {
		...
		if (reg.states) {
			for(const field of reg.states) {
				...
				if (field.store !== storeType.never && !reg['initState'+this.inverterInfo.index]) {
					//state
					/*
					if (reg.address == 37000) {
						console.log('+++++ '+reg.address+' '+reg.info+' +++++');
...
class InverterInfo extends InverterInstance {
	constructor (stateInstance,inverter) {
		...
		const newFields = [
			{
				...
				postHook: (path) => {
					...
					if (modelId) this.inverterInfo.modelId = modelId;
				}
			}
		];
...
class Sun2000_Serie_M0 extends InverterInstance{
	constructor(stateInstance,inverter) {
		super(stateInstance,inverter);

		this._modelId = 428; //sun2000_M1
		...
		const newFields = [
			{
				...
				states : [{
					...
					register: { reg: 32064, type: dataType.int32, gain: 1000, length: 0 },
				...
				},
				{
					...
					register: {	reg: 32080, type: dataType.int32, gain: 1000, length: 0 },
					store : storeType.always
				}
				],
				postHook: (path) => {
					...
					this.solarSum.add(inPowerEff); //riemannSum
				}
			},
			{
				address : 37052,
...
class Sun2000_Serie_M1 extends Sun2000_Serie_M0{
	constructor(stateInstance,inverter) {
		super(stateInstance,inverter);
		...
		const newFields = [
			...
			},
			{
				...
				readErrorHook: (reg) => {
					//modbus Error 2 - illegal address
					//reg couldnt read
					//this.adapter.log.debug('####### readErrorHook ######');
					reg['lastread'+this.inverterInfo.index] = new Date().getTime();
...
function getInverterHandler(modelId) {
	...
	if (model_sun2000M0.includes(modelId)) return Sun2000_Serie_M0;
	if (model_sun2000M1.includes(modelId)) return Sun2000_Serie_M1;
}

module.exports = {InverterInfo, getInverterHandler};
