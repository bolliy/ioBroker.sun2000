...
class statistics {
	constructor(adapterInstance, stateCache) {
		...
		this.stats = [
			...
			},
			{
				...
				compute: entry => {
					let solarYield = (entry.solarYield?.total != null ? entry.solarYield?.total : entry.gridExport?.value) ?? 0;
					solarYield += (entry.dailyExternalYield?.total != null ? entry.dailyExternalYield?.total : entry.dailyExternalYield?.value) ?? 0;
	 ...
	 * @returns {boolean} true if a new entry was appended, false otherwise
	 */
	_calculateAggregation_t1(sourceStateId, targetStateId, getWindow, periodType) {
		try {
			const now = new Date();
			...
			const existingIdx = targetArray.findLastIndex(e => e.from === fromStr); //faster!
		...
		const tooltipFormatter = params => {
			...
			return params
				...
				.map(p => {
					...
					const seriesName = ['Self-sufficiency', 'Self-consumption'].includes(p.seriesName) ? `${p.seriesName} today` : p.seriesName;
