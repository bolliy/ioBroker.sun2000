...
class statistics {
		...
		this.postProcessHooks = [
			{
				...
				states: [
					...
					// Today summary state
					{
						...
						type: 'string',
						role: 'json',
						desc: "Live summary of today's energy values, updated every hour",
						initVal: '{}',
					},
	...
	/**
	 * Updates the statistics.today state with the current live day values.
	...
	_updateToday() {
		try {
			...
			this.stateCache.set('statistics.today', todayStr, { type: 'string' });
			this.adapter.logger.debug('statistics: today state updated');
		} catch (err) {
	...
	_calculateHourly() {
		...
		// Always update today state every hour
		this._updateToday();
	}
	...
	async initialize() {
		...
		state = await this.adapter.getState('statistics.today');
		this.stateCache.set('statistics.today', state?.val ?? '{}', { type: 'string', stored: true });
