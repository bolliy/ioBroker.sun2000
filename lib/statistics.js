...
class statistics {
	...
	async _calculateGeneric(stateId, periodStart) {
		const now = new Date();
	...
	async _calculateHourly() {
		...
		const lastHour = new Date(now.getFullYear(), now.getMonth(), now.getDate(), now.getHours() - 1, 0, 0, 0);
		if (now < lastHour) {
			this.adapter.logger.debug('statistics.js: Skipping hourly calculation because current time is before end of window');
			return;
		}
		await this._calculateGeneric('statistics.jsonHourly', startOfDay);
	 ...
	 * @returns {void}
	 */
	async _calculateAggregation(sourceStateId, targetStateId, getWindow, getRetentionStart, periodType) {
		try {
			const now = new Date();
	...
	async _executeScheduledTasks() {
		...
		await this._clearGeneric('statistics.jsonHourly', periodStart);
	}
	...
	async initialize() {
		...
		// wait until consumptionToday is available to avoid running the task before the initial state is loaded
		await tools.waitForValue(() => this.stateCache.get('collected.consumptionToday')?.value, 30000);
