...
const { reviveFunctions } = require(`${__dirname}/json_helper.js`);
...
class statistics {
	...
	_buildFlexchart(myChart, chartStyle) {
		...
		try {
			...
			if (Object.keys(templ).length === 0) {
				template = this._buildDefaultChart(myChart, chartStyle, xAxisData, xAxisDataShort, dayAreas);
			} else {
				...
				//chartStr = stringifyWithFunctions(templ);
			}
		} catch (e) {
		...
		if (template?.dataZoom?.[0] && !template.dataZoom[0].start && !template.dataZoom[0].end) {
			template.dataZoom[0].start = slider.start;
			template.dataZoom[0].end = slider.end;
		}

		let chartStr = stringify(template);
	 ...
	 * @param dayAreas
	 */
	_buildDefaultChart(myChart, chartStyle, xAxisData, xAxisDataShort, dayAreas) {
		/*
		...
		*/
		...
		//return stringifyWithFunctions(chart).replace('"%%xAxisFormatter%%"', stringifyWithFunctions(xAxisFormatterHourly));
		return chart; //as object, not stringified, since we use reviveFunctions to restore the functions after stringification in the state
	}
