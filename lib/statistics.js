...
class statistics {
		...
		this.postProcessHooks = [
			{
				...
				states: [
					...
					{
						id: 'statistics.flexChart.template',
					...
					{
						id: 'statistics.flexChart.jsonOutput',
						name: 'Flexcharts outputJSON string to be used for FlexCharts adapter for Apache ECharts',
						type: 'string',
						role: 'json',
						desc: '',
						initVal: '{}',
	...
	async initialize() {
		...
		state = await this.adapter.getState('statistics.flexChart.template');
		this.stateCache.set('statistics.flexChart.template', state?.val ?? '{}', { type: 'string', stored: true });
