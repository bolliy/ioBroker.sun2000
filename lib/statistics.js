...
class statistics {
	 ...
	 */
	async _executeScheduledTasks_old() {
		const now = new Date();
	...
	async initialize() {
		...
		await tools.waitForValue(() => this.stateCache.get('collected.dailySolarYield')?.value, 30000);
		await tools.waitForValue(() => this.stateCache.get('collected.SOC')?.value, 30000);
