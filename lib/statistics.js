...
class statistics {
	...
	/**
	 * Generic aggregation function for daily/weekly/monthly/annual statistics.
	 * Sums values from source over a period window and stores in target.
	 *
	 * @param {string} sourceStateId - Source state ID (e.g., jsonHourly or jsonDaily)
	 * @param {string} targetStateId - Target state ID (e.g., jsonDaily, jsonWeekly)
	 * @param {Function} getWindowStart - Returns start date of period to aggregate
	 * @param {Function} getRetentionStart - Returns earliest date to keep in target
	 * @param {string} periodType - Type name for logging
	 */
	async _calculateAggregation(sourceStateId, targetStateId, getWindowStart, getRetentionStart, periodType) {
		try {
			const now = new Date();
			...
			let sum = 0;
			if (periodType === 'daily') {
				sum = sourceEntries.length > 0 ? sourceEntries[sourceEntries.length - 1]?.consumptionToday : 0;
			} else {
				try {
