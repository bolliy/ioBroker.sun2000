...
class statistics {
	...
	_buildFlexchart2(myChart, chartStyle = 'bar') {
	...
	_buildFlexchart3(myChart) {
		const xAxisFormatterHourly = value => {
			// value ist z.B. "14.03.2025 08:00" oder "08:00"
			if (value.includes('|')) {
				return value; // Datumswechsel-Label unverändert
			}
			return value.split(' ')[1] ?? value;
		};
		/*
		const xAxisFormatterH1 = (value, index, extra) => {
			try {
		...
		const xAxisDataShort =
			myChart === 'hourly'
				? xAxisData.map((label, i) => {
						...
						const date = label.split(' ')[0];
						const prevDate = i > 0 ? xAxisData[i - 1].split(' ')[0] : null;
						// Ersten Eintrag und jeden Datumswechsel mit Datum markieren
						return i === 0 || date !== prevDate ? `${date} | ${time}` : time;
					})
				: xAxisData;
		*/
			...
			xAxis: [
				{
					...
					type: 'category',
					data: xAxisData, // <-- kurzes Array ohne Datum
					scale: true,
					boundaryGap: false,
					axisLine: { onZero: false },
					splitLine: { show: false },
					...
					axisLabel: {
						...
						lineHeight: 16,
						fontSize: 11,
						formatter: '%%xAxisFormatter%%', // Platzhalter
					},
				},
				{
	...
	handleFlexMessage(message, callback) {
		...
		const result = this._buildFlexchart2(chartType, chartStyle);
