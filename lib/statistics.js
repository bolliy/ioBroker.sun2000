...
class statistics {
		...
		this.postProcessHooks = [
			{
				...
				states: [
					...
					{
						id: 'statistics.flexChart.template.hourly',
					...
					{
						id: 'statistics.flexChart.template.daily',
					...
					{
						id: 'statistics.flexChart.template.weekly',
					...
					{
						id: 'statistics.flexChart.template.monthly',
					...
					{
						id: 'statistics.flexChart.template.annual',
					...
					{
						id: 'statistics.flexChart.jsonOutput.hourly',
					...
					{
						id: 'statistics.flexChart.jsonOutput.daily',
					...
					{
						id: 'statistics.flexChart.jsonOutput.weekly',
					...
					{
						id: 'statistics.flexChart.jsonOutput.monthly',
					...
					{
						id: 'statistics.flexChart.jsonOutput.annual',
	...
	renewalTemplate(myChart, state) {
		const templateStateId = `statistics.flexChart.template.${myChart}`;
		const template = this.stateCache.get(templateStateId)?.value;
		if (template === null || template === undefined) {
			this.adapter.logger.warn(`Template state ${templateStateId} not found for renewal`);
			return;
		}
		if (state?.val !== null) {
			this.adapter.logger.debug(`statistics: Event - state: ${myChart} changed: ${state.val} ack: ${state.ack}`);
			this.stateCache.set(templateStateId, state.val, { type: 'string', stored: true });
			this._buildFlexchart(myChart, 'line');
		}
	}
	...
	async _calculateHourly() {
		...
		if (this._calculateGeneric('statistics.jsonHourly', startOfDay, lastHour)) {
			...
			this._buildFlexchart('hourly', 'line');
		}
	}
	...
	async _calculateDaily() {
		...
		this._calculateAggregation(
			...
			'daily',
		) && this._buildFlexchart('daily', 'line'); // only update last execution time if aggregation was performed to avoid backfilling multiple days at startup
	}
	...
	async _calculateWeekly() {
		...
		this._calculateAggregation(
			...
			'weekly',
		) && this._buildFlexchart('weekly', 'line'); // only update last execution time if aggregation was performed to avoid backfilling multiple weeks at startup
	}
	...
	async _calculateMonthly() {
		...
		this._calculateAggregation(
			...
			'monthly',
		) && this._buildFlexchart('monthly', 'line'); // only update last execution time if aggregation was performed to avoid backfilling multiple months at startup
	}
	...
	async _calculateAnnual() {
		...
		this._calculateAggregation(
			...
			'annual',
		) && this._buildFlexchart('annual', 'line'); // only update last execution time if aggregation was performed to avoid backfilling multiple years at startup
	}
	...
	async initialize() {
		...
		for (const chartType of ['hourly', 'daily', 'weekly', 'monthly', 'annual']) {
			state = await this.adapter.getState(`statistics.flexChart.template.${chartType}`);
			this.stateCache.set(`statistics.flexChart.template.${chartType}`, state?.val ?? '{}', { type: 'string', stored: true });
		}
	...
	_buildFlexchart(myChart, chartStyle = 'bar') {
		...
		const templateStateId = `statistics.flexChart.template.${myChart}`;
		const outputStateId = `statistics.flexChart.jsonOutput.${myChart}`;
		...
		// const test = JSON.stringify(xAxisDataShort);
		...
		this.adapter.logger.debug(`statistics: flexChart built for ${myChart}/${chartStyle}`);
	 ...
	 *
	 * @param {string} myChart - one of 'hourly','daily','weekly','monthly','annual'
	 * @param {string} chartStyle - 'line', 'bar'
	 * @returns {object} chart configuration
	 */
	_buildFlexchart_old(myChart, chartStyle = 'line') {
		const xAxisFormatterHourly = value => {
			// value ist z.B. "14.03.2025 08:00" oder "08:00"
			if (value.includes('|')) {
				return value;
			}
			return value.split(' ')[1] ?? value;
		...
		// --- X-Axis labels mit Datum+Uhrzeit für den Slider ---
		const xAxisData = data.map(entry => {
			const from = new Date(entry.from);
			return myChart === 'hourly'
				? `${from.toLocaleDateString('de-DE', { year: 'numeric', month: '2-digit', day: '2-digit' })} ${from.toLocaleTimeString('de-DE', {
		...
		const xAxisDataShort = myChart === 'hourly' ? xAxisData.map(label => label.split(' ')[1]) : xAxisData;
		/*
		// Hauptachse: Uhrzeit, bei Datumswechsel "DD.MM.YYYY | HH:MM"
		const xAxisDataShort =
			myChart === 'hourly'
				? xAxisData.map((label, i) => {
						const date = label.split(' ')[0];
						const time = label.split(' ')[1];
						const prevDate = i > 0 ? xAxisData[i - 1].split(' ')[0] : null;
						return i === 0 || date !== prevDate ? `${date} | ${time}` : time;
					})
				: xAxisData;
		*/

		// --- Tagesbereiche berechnen: abwechselnd schraffiert/unschraffiert ---
		const dayAreas = [];
		if (myChart === 'hourly' && xAxisData.length > 0) {
			const dayBoundaries = [0];
			xAxisData.forEach((label, i) => {
		...
		// --- Series type basierend auf chartStyle ---
		const seriesType = chartStyle === 'line' ? 'line' : 'bar';
		const lineOptions =
			chartStyle === 'line'
				? {
						...
						lineStyle: { width: 2 },
						areaStyle: {
							opacity: 0.15, // leichter Farbverlauf
						},
					}
				: {};
		...
		const chart = {
			...
			xAxis: [
				{
					...
					data: xAxisDataShort,
					scale: true,
					boundaryGap: chartStyle !== 'line', // false bei line, true bei bar
					axisLine: { onZero: false },
					splitLine: { show: false },
					axisPointer: { z: 100 },
					min: 0,
			...
			series: [
				...
				// Consumption im unteren Grid — immer bar
				{
					name: 'Consumption',
					type: seriesType,
					xAxisIndex: 1,
				...
				// Tages-Bereiche — nur bei hourly und wenn Daten vorhanden
				...(dayAreas.length > 0
					? [
							{
								name: 'DayBreak',
		...
		const templateStr = this.stateCache.get('statistics.flexChart.template')?.value;
		if (templateStr) {
			try {
				const templ = JSON.parse(templateStr);
				for (const key of Object.keys(templ)) {
					// Speziell: series wird nach name zusammengeführt
					if (key === 'series' && Array.isArray(templ[key]) && Array.isArray(chart[key])) {
						templ[key].forEach(templSeries => {
							if (!templSeries.name) return;
							const existing = chart[key].find(s => s.name === templSeries.name);
							if (existing) {
								// Vorhandene Serie tief zusammenführen
								this._deepMerge(existing, templSeries);
							} else {
								// Neue Serie anhängen
								chart[key].push(templSeries);
							}
						});
						// Speziell: Arrays wie xAxis, yAxis, grid nach Index zusammenführen
					} else if (Array.isArray(templ[key]) && Array.isArray(chart[key])) {
						templ[key].forEach((item, i) => {
							if (chart[key][i] && typeof item === 'object') {
								this._deepMerge(chart[key][i], item);
							} else {
								chart[key][i] = item;
							}
						});
						...
						templ[key] !== null &&
						typeof templ[key] === 'object' &&
						chart[key] !== null &&
						typeof chart[key] === 'object' &&
						!Array.isArray(templ[key])
					) {
						this._deepMerge(chart[key], templ[key]);
		...
		const chartStr = stringify(chart).replace("'%%xAxisFormatter%%'", stringify(xAxisFormatterHourly));
	...
	handleFlexMessage(message, callback) {
		...
		const chartStyle = message?.style || 'line'; // 'bar' oder 'line'
