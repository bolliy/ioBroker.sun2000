...
class statistics {
	...
	_buildDefaultChart(myChart, chartStyle, xAxisData, xAxisDataShort, dayAreas, seriesData) {
		...
		// For hourly: show full range (data spans max 2 days)
		// For daily: show last 30 entries (~1 month)
		// For weekly: show last 12 entries (~3 months)
		// For monthly: show last 13 entries (~1 year)
		// For annual: show full range
		const sliderDefaults = {
			hourly: { start: 0, end: 100 },
			daily: { start: Math.max(0, Math.round((1 - 30 / Math.max(xAxisData.length, 1)) * 100)), end: 100 },
			weekly: { start: Math.max(0, Math.round((1 - 12 / Math.max(xAxisData.length, 1)) * 100)), end: 100 },
