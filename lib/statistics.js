...
// ---------------------------------------------------------------------------
const CHART_TYPES = ['hourly', 'daily', 'weekly', 'monthly', 'annual'];
...
class statistics {
	constructor(adapterInstance, stateCache) {
		...
		this.testing = false; // set to true for unit-testing
	...
	_calculateGeneric(stateId, periodStart, periodEnde) {
		...
		// Pass 3: subtract breakdown values (unit: kWh) from consumption so that
		// breakdown series + remainder stack up exactly to the total consumption.
		// Only breakdown entries with unit 'kWh' are subtracted; others are skipped.
	...
	updateJsonToday() {
		...
		try {
			...
			// Subtract breakdowns from consumption to get the remainder for the upper sub-chart.
			if (today.consumption !== undefined) {
				let breakdownSum = 0;
				for (const stat of this._breakdown.statsEntries) {
					if (!stat._isBreakdown) continue;
					if ((stat.unit || 'kWh') !== 'kWh') continue;
					breakdownSum += Number(today[stat.targetPath]?.value ?? 0);
				}
				if (breakdownSum > 0) {
					const remainder = Math.max(0, Math.round((Number(today.consumption.value) - breakdownSum + Number.EPSILON) * 1000) / 1000);
					today.consumption = { ...today.consumption, value: Number(remainder.toFixed(3)) };
					this.adapter.logger.debug(`updateJsonToday: consumption reduced by breakdown sum ${breakdownSum} → remainder ${remainder}`);
				}
			}
	...
	async initialize() {
		...
		for (const chartType of CHART_TYPES) {
			...
			const state = await this.adapter.getStateAsync(templateStateId);
			...
			if (state?.ack === false) {
				...
				await this.adapter.setStateAsync(templateStateId, { val: state.val, ack: true });
	...
	_buildFlexchart(myChart, chartStyle) {
		...
		try {
			...
			if (Object.keys(parsed).length === 0 || command === 'createTemplateFromBuiltin') {
				...
				//template = reviveFunctions(parsed);
				template = parsed;
					...
					? [
							{
								...
								left: 'center',
								top: 'middle',
								style: { text: this._noDataHint(myChart), fontSize: 14, fill: '#999' },
							},
						]
					: [],
		};
	...
	_buildDefaultTemplate(myChart, chartStyle) {
		...
		// Legend: built-in series names, then breakdown names appended by chart helper
		const baseLegend = [
			'Solar Yield',
			'Grid Export',
			'Grid Import',
		...
		return {
			...
			yAxis: [
				// 0 — Energy (kWh), left axis, upper grid
				{
					scale: false,
					splitArea: { show: true },
					name: 'Energy (kWh)',
				...
				// 1 — SOC / ratio (%), right axis, upper grid
				{
					type: 'value',
					min: 0,
					max: 100,
				...
				// 2 — Consumption (kWh), lower sub-grid
				{
					scale: true,
					gridIndex: 1,
					splitNumber: 3,
			...
			series: [
				...
				// Breakdown stacked — delegated to chart helper
				...chartHelper.buildBreakdownSeries(this._breakdown.entries, seriesType, lineOptions),
				// Lower sub-grid — residual consumption (total minus breakdowns), same stack as breakdown series
				{
					name: 'Consumption',
					type: seriesType,
	...
	async handleTemplateChange(chartType, state) {
		...
		if (state?.val != null) {
			...
			this.stateCache.set(templateStateId, state.val, { type: 'string', stored: true });
			this._buildFlexchart(chartType);
			await this.adapter.setStateAsync(templateStateId, { val: this.stateCache.get(templateStateId)?.value, ack: true });
		}
	}
