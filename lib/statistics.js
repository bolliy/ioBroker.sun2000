...
class statistics {
	...
	async initialize() {
		...
		for (const chartType of ['hourly', 'daily', 'weekly', 'monthly', 'annual']) {
			state = await this.adapter.getState(`statistics.flexCharts.template.${chartType}`);
			this.stateCache.set(`statistics.flexCharts.template.${chartType}`, state?.val ?? '{}', { type: 'string', stored: true });
		}
	...
	async handleTemplateChange(chartType, state) {
		...
		if (state?.val != null) {
			...
			this._buildFlexchart(chartType, 'line');
		}
	}
