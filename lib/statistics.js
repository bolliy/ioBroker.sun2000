...
class statistics {
	constructor(adapterInstance, stateCache) {
		...
		this.stats = [
			...
			{ sourceId: 'collected.dailyInputYield', targetPath: 'dailyInputYield', unit: 'kWh', type: statisticsType.deltaReset },
			{
	...
	async _calculateGeneric(stateId, periodStart, periodEnde) {
		...
		for (const stat of this.stats) {
			...
			if (stat.type === statisticsType.delta || stat.type === statisticsType.deltaReset) {
				...
				// avoid delta-reset if value is less than 5% of last value
				if (stat.type === statisticsType.deltaReset) {
					if (value >= lastTotal * 0.95) {
						// Wert ist gestiegen –> Delta-Berechnung, da wahrscheinlich kein tagesreset stattgefunden hat
						value -= lastTotal;
					}
				} else {
