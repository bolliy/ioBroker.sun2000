	...
	constructor(adapterInstance, stateCache) {
		...
		this.stats = [
			...
			},
			{
				...
				compute: entry => {
					let yield = (entry.solarYield?.total != null ? entry.solarYield?.total : entry.gridExport?.value) ?? 0;
					yield += (entry.dailyExternalYield?.total != null ? entry.dailyExternalYield?.total : entry.dailyExternalYield?.value) ?? 0;
					const gridExport = (entry.gridExport?.total != null ? entry.gridExport?.total : entry.gridExport?.value) ?? 0;
					if (yield <= 0) return 0;
					return Math.round(Math.max(0, Math.min(100, (1 - gridExport / yield) * 100)) * 10) / 10;
				},
			},
		];
