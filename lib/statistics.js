	...
	_buildFlexchart(myChart, chartStyle) {
		...
		const chart = {
			...
			series: [
				...
				...chartHelper.buildBreakdownSeries(this._breakdown.entries, seriesData),
			],
			graphic:
				xAxisData.length === 0
					? [
	...
	_buildDefaultTemplate(myChart, chartStyle) {
		...
		return {
			...
			series: [
				...
				// Lower sub-grid — total consumption reference
				{
					name: 'Consumption',
					type: seriesType,
					xAxisIndex: 1,
				...
				// Breakdown series injected dynamically in _buildFlexchart() — not here.
			],
		};
	}
