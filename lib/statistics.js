...
class statistics {
	...
	_buildFlexchart(myChart, chartStyle) {
		...
		/*
		// If template has xAxis without min/max, set it to show all data
		if (chart?.xAxis?.[0] && !chart.xAxis[0].min && !chart.xAxis[0].max) {
			chart.xAxis[0].min = 0;
			chart.xAxis[0].max = xAxisDataShort.length - 1;
		}
		if (chart?.yAxis?.[1] && !chart.yAxis[1].min && !chart.yAxis[1].max) {
			chart.yAxis[1].min = 0;
			chart.yAxis[1].max = xAxisData.length - 1;
		}
		*/
