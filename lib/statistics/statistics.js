...
class statistics {
	...
	_calculateLive() {
		...
		for (const stat of this._effectiveLiveStats()) {
			...
			if (stat.targetPath === 'consumption') {
				this.adapter.log.info(
					`### Live stat from ${last.from} to ${last.to} - ${stat.targetPath}, total: ${last[stat.targetPath]?.total} ${last[stat.targetPath].value} kWh → ${kw} kW (convFactor ${convFactor.toFixed(2)})`,
				);
			}
