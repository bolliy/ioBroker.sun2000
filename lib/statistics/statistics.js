...
class statistics {
	...
	_buildFlexchart(myChart, chartStyle) {
		...
		const tooltipFormatter = params => {
			/*
			const formatSeriesValue = (series, value) => {
				let v = value;
				if (series.invertSign) {
					v *= -1;
				}
				//return `${v.toFixed(series.decimals ? series.decimals : 3)} ${series.unit ?? ''}`;
				return `${v.toFixed(series.decimals ? series.decimals : 3)} ${series.unit ?? ''}`;
			}; 
			*/
			...
			return params
				...
				.map(p => {
					//const series = template.series?.find(s => s.name === p.seriesName);
					//return `${p.marker}${p.seriesName}: <b>${formatSeriesValue(p.series, p.value)}</b>`;
					console.log(`p.series: ${JSON.stringify(p.series)}`);
					return `${p.marker}${p.seriesName}: <b>${JSON.stringify(p?.series)}</b>`;
				})
				.join('<br/>');
		};
	...
	_buildDefaultTemplate(myChart, chartStyle) {
		...
		const formatValue = unit => `value => \`\${value} ${unit}\``;
		...
		return {
			...
			series: [
				...
				},
				{
					...
					emphasis: { focus: 'series' },
					tooltip: { valueFormatter: formatValue(unit) },
					...lineOptions,
				},
				{
					name: 'Grid Import',
				...
				...chartHelper.buildBreakdownSeries(this._breakdown.entries, seriesType, lineOptions, unit),
				{
					name: 'Consumption',
					type: seriesType,
