 ...
 *   lib/consumptionBreakdown.js  – config, validation, foreign-state refresh
 *   lib/breakdownChartHelper.js  – eCharts series / legend / placeholder helpers
 *
 * Live chart:
 *   Uses the same this.stats data pipeline as the hourly chart.
 *   Every LIVE_INTERVAL_MINUTES the kWh delta is converted to average kW:
...
// kW = ΔkWh * (60 / LIVE_INTERVAL_MINUTES)
// ---------------------------------------------------------------------------
const LIVE_INTERVAL_MINUTES = 5; // change to 1, 5, 15, ... as needed
...
class statistics {
		...
		this.postProcessHooks = [
			{
				...
				states: [
					// [LIVE] state definitions
					{
						...
						type: 'string',
						role: 'json',
						desc: `Live power (kW) chart data, ${LIVE_INTERVAL_MINUTES}-min intervals, 48 h retention`,
						initVal: '[]',
					},
					{
	...
	_calculateGeneric(stateId, periodStart, periodEnde) {
		...
		for (const stat of effectiveStats) {
			...
			if (source === null || source === undefined) {
				if (!stat._isBreakdown) this.adapter.logger.warn(`Source state ${stat.sourceId} not found statistic hook`);
				continue;
			}
	 ...
	 * @param {Function} getWindow      (now: Date) => { from: Date, to: Date }
	...
	_calculateLive() {
		...
		const intervalMs = LIVE_INTERVAL_MINUTES * 60 * 1000;
		...
		const appended = this._calculateGeneric(CHART_STATE_IDS.live, startOfDay, boundary);
		if (!appended) return;
		...
		for (const stat of this._effectiveStats()) {
			...
			//last[stat.targetPath] = { value: kw, unit: 'kW' };
			last[stat.targetPath] = { ...last[stat.targetPath], value: kw, unit: 'kW', originalValue: last[stat.targetPath].value, confFactor: convFactor };
			/*last[stat.targetPath].value = kw;
			last[stat.targetPath].unit = 'kW';
			*/
			// 'total' (cumulative kWh) is intentionally dropped — not meaningful for kW display
		}
		...
		*/
		...
		this.stateCache.set(CHART_STATE_IDS.live, JSON.stringify(arr), { type: 'string' });
		this._buildFlexchart('live');
	}

	// [LIVE] Minute-aligned timer, fires every LIVE_INTERVAL_MINUTES
	_initializeLiveTimer() {
		const scheduleNext = () => {
			const now = new Date();
			const intervalMs = LIVE_INTERVAL_MINUTES * 60 * 1000;
			// Next boundary strictly in the future
			const nextBoundary = new Date(Math.floor(now.getTime() / intervalMs) * intervalMs + intervalMs);
			const ms = nextBoundary.getTime() - now.getTime();

			if (this._liveTimer) this.adapter.clearTimeout(this._liveTimer);
			this._liveTimer = this.adapter.setTimeout(() => {
				this._calculateLive();
				scheduleNext();
			}, ms);

			this.adapter.logger.debug(`statistics: live timer set, interval=${LIVE_INTERVAL_MINUTES} min, next run in ${Math.round(ms / 1000)} s`);
		};
		scheduleNext();
	}
	...
	_buildFlexchart(myChart, chartStyle) {
		// [LIVE] delegate to dedicated builder
		//if (myChart === 'live') return this._buildFlexchartLive();
		...
		const xAxisData = data.map(entry => {
			...
			if (myChart === 'live') {
				//only full hours get a label, to avoid clutter. Label format: "dd.MM. HH:mm" (e.g. "01.08. 14:00")
				if (to.getMinutes() === 0) {
					return `${to.toLocaleDateString('de-DE', { day: '2-digit', month: '2-digit' })} ${to.toLocaleTimeString('de-DE', { hour12: false, hour: '2-digit' })}`;
				}
				return ``;
			}
			if (myChart === 'hourly') {
		...
		const xAxisDataShort = myChart === 'hourly' || myChart === 'live' ? xAxisData.map(l => l.split(' ')[1]) : xAxisData;
		...
		const xAxisFormatter = value => (value.includes('|') ? value : (value.split(' ')[1] ?? value));
		const tooltipFormatter = params => {
			if (!Array.isArray(params)) params = [params];
			return params
				.filter(p => p.seriesName !== 'DayBreak')
				.map(p => {
					if (typeof p.value === 'object') return `${p.marker}${p.seriesName}: <b>${p.value.value}${p.value.unit || 'kWh'}</b>`;
					const negated = ['Grid Export', 'Charge'].includes(p.seriesName);
					const val = negated ? Math.abs(p.value) : p.value;
					...
					return `${p.marker}${name}: <b>${val}${unit}</b>`;
				})
				.join('<br/>');
		};
		...
		const chart = {
			...
			graphic:
				xAxisData.length === 0
					? [{ type: 'text', left: 'center', top: 'middle', style: { text: this._noDataHint(myChart), fontSize: 14, fill: '#999' } }]
					: [],
		};

		// Apply slider start/end defaults when the template has no explicit values
		const sliderDefaults = {
			live: Math.max(0, Math.round((1 - 25 / Math.max(xAxisData.length, 1)) * 100)),
	...
	// [LIVE] Dedicated chart builder for the live (kW) view
	/**
	 * Builds the eCharts config for statistics.jsonLive.
	 *
	 * The data already contains kW values (converted in _calculateLive).
	 * X-axis labels are HH:MM; day boundaries are marked with a dashed line.
	 * The slider shows the last 24 h by default.
	 *
	 * @returns {string} Stringified eCharts configuration
	 */
	_buildFlexchartLive() {
		const outputStateId = 'statistics.flexCharts.jsonOutput.live';
		const templateStateId = 'statistics.flexCharts.template.live';

		let data = [];
		try {
			data = JSON.parse(this.stateCache.get(CHART_STATE_IDS.live)?.value ?? '[]');
		} catch {
			data = [];
		}

		// X-axis: "DD.MM. HH:MM" so day boundaries are visible in the tooltip;
		// short label (HH:MM) used on the axis itself.
		const xAxisData = data.map(e => {
			const d = new Date(e.to);
			return `${d.toLocaleDateString('de-DE', { day: '2-digit', month: '2-digit' })} ${d.toLocaleTimeString('de-DE', { hour12: false, hour: '2-digit', minute: '2-digit' })}`;
		});
		const xAxisDataShort = xAxisData.map(l => l.split(' ')[1]);

		// Day-boundary shading (same logic as hourly)
		const dayAreas = [];
		...
		// Series: one line per deltaReset stat that was converted to kW.
		// SOC (level/%) and computed (%) get their own right-hand axis.
		const energyColors = {
			consumption: '#337ab7',
			solarYield: '#f6c94e',
			inputYield: '#e8a000',
			energyYield: '#ffa500',
			externalYield: '#8bc34a',
			chargeCapacity: '#5bc0de',
			dischargeCapacity: '#ed50e0',
			gridExport: '#5cb85c',
			gridImport: '#ec0000',
		};
		...
		for (const stat of this.stats) {
			const values = data.map(e => e[stat.targetPath]?.value ?? null);
			const color = energyColors[stat.targetPath] ?? '#999';

			if (stat.type === statisticsType.level || stat.type === statisticsType.computed) {
				// SOC / ratios → right axis (yAxisIndex 1)
				pctSeries.push({
					name: stat.targetPath,
					type: 'line',
					yAxisIndex: 1,
					smooth: true,
					symbol: 'none',
					...
					itemStyle: { color },
					data: values,
					tooltip: { valueFormatter: v => `${v} ${stat.unit}` },
				});
			} else {
				// kW series → left axis (yAxisIndex 0)
				kwSeries.push({
					name: stat.targetPath,
					type: 'line',
					smooth: true,
		...
		const tooltipFormatter = params => {
			...
			return params
				.filter(p => p.seriesName !== 'DayBreak')
				.map(p => {
					const unit = p.seriesName === 'SOC' ? ' %' : ' kW';
					return `${p.marker}${p.seriesName}: <b>${p.value ?? '–'}${unit}</b>`;
				})
				.join('<br/>');
		};

		// Slider: default to last 24 h worth of points
		const pointsPer24h = Math.round((24 * 60) / LIVE_INTERVAL_MINUTES);
		const sliderStart = Math.max(0, Math.round((1 - pointsPer24h / Math.max(data.length, 1)) * 100));
		...
		try {
			...
			command = parsed.command || '';
			if (Object.keys(parsed).length === 0 || command === 'createTemplateFromBuiltin') {
				template = this._buildDefaultTemplateLive();
			} else {
				template = parsed;
				delete template._meta;
			}
		} catch (e) {
			this.adapter.logger.warn(`statistics: invalid template for live: ${e.message}`);
			template = this._buildDefaultTemplateLive();
		}
		...
		const chart = {
			...
			graphic:
				data.length === 0
					? [{ type: 'text', left: 'center', top: 'middle', style: { text: this._noDataHint('live'), fontSize: 14, fill: '#999' } }]
					: [],
		};

		if (chart?.dataZoom?.[0] && !chart.dataZoom[0].start && !chart.dataZoom[0].end) {
			chart.dataZoom[0].start = sliderStart;
			chart.dataZoom[0].end = 100;
		}
		...
		this.stateCache.set(outputStateId, chartStr, { type: 'string', renew: command === 'createTemplateFromBuiltin' });
		this.adapter.logger.debug(`statistics: flexCharts built for live (${LIVE_INTERVAL_MINUTES}-min interval)`);

		if (command === 'createTemplateFromBuiltin') {
			template = { _meta: { generatedFrom: 'builtin', generatedAt: new Date().toISOString() }, ...this._buildDefaultTemplateLive() };
			this.stateCache.set(templateStateId, stringifyWithFunctions(template), { type: 'string' });
		}
	...
	// [LIVE] Default template skeleton — series are injected dynamically
	_buildDefaultTemplateLive() {
		return {
			backgroundColor: '#fff',
			animation: false,
			title: { left: 'center', text: `PV Live Power — kW (${LIVE_INTERVAL_MINUTES}-min avg)` },
			legend: { top: 35, left: 'center' },
			tooltip: {
				trigger: 'axis',
			...
			grid: [{ left: '8%', right: '8%', top: 80, bottom: 60 }],
			xAxis: [
				{
					type: 'category',
					data: '%%xAxisDataShort%%',
					...
					axisPointer: { z: 100 },
					axisLabel: { interval: 0, lineHeight: 16, fontSize: 11, formatter: value => value },
					min: 0,
					max: '%%xAxisMax%%',
				},
			],
			...
			dataZoom: [
				...
				{ show: true, xAxisIndex: [0], type: 'slider', bottom: 5 },
			],
			series: [], // filled dynamically in _buildFlexchartLive
		};
	}
	...
	_noDataHint(myChart) {
		return (
			{
				// [LIVE] added
				live: `No data yet — first entry available after ${LIVE_INTERVAL_MINUTES} minute(s).`,
	...
	_buildDefaultTemplate(myChart, chartStyle) {
		...
		const showSOC = myChart === 'hourly' || myChart === 'live';
			...
			xAxis: [
				{
					...
					splitLine: { show: false },
					axisPointer: { z: 100 },
					axisLabel: { interval: 0, lineHeight: 16, fontSize: 11, formatter: '%%xAxisFormatter%%' },
					min: 0,
					max: '%%xAxisMax%%',
				},
				{
			...
			yAxis: [
				{
					scale: false,
					splitArea: { show: true },
					name: myChart === 'live' ? 'Energy (kWh)' : 'Energy (kW)',
					nameLocation: 'middle',
					nameGap: 50,
					axisLabel: { formatter: myChart === 'live' ? '{value} kWh' : '{value} kW' },
					splitLine: { show: true },
					axisLine: { show: true },
				},
				{
			...
			series: [
				{
					...
					tooltip: { valueFormatter: v => ({ value: v, unit: myChart === 'live' ? 'kWh' : 'kW' }) },
					...lineOptions,
				},
				{
					name: 'Grid Export',
					...
					? [
							{
								...
								symbol: 'none',
								smooth: true,
								tooltip: { valueFormatter: v => ({ value: v, unit: '%' }) },
							},
						]
					: []),
				{
				...
				...chartHelper.buildBreakdownSeries(this._breakdown.entries, seriesType, lineOptions),
				{
					name: 'Consumption',
					type: seriesType,
