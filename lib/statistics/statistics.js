...
class statistics {
	...
	_calculateLive() {
		...
		for (const [index, stat] of this._effectiveLiveStats()) {
			...
			if (index === 0) {
				this.adapter.log.info(`### Live stat ${stat.targetPath}: ${last[stat.targetPath].value} kWh → ${kw} kW (convFactor ${convFactor.toFixed(2)})`);
			}
