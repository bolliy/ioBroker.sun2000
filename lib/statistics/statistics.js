...
class statistics {
	constructor(adapterInstance, stateCache) {
		...
		this.stats = [
			...
			// --- Computed ---
			{
				...
				compute: entry => {
					const consumption = entry.consumption?.total ?? entry.consumption?.baseValue ?? entry.consumption?.value ?? 0;
		...
		this.postProcessHooks = [
			{
				...
				states: [
					...
					// Breakdown config + internal day-start baseline states — definitions provided by sub-module
					...this._breakdown.stateDefs,
	...
	_calculateGeneric(stateId, periodStart, periodEnde, effectiveStats = this._effectiveStats()) {
		...
		if (entry.consumption?.value !== undefined) {
			entry.consumption = { ...entry.consumption, baseValue: entry.consumption.value };
			let breakdownSum = 0;
			...
			for (const stat of effectiveStats) {
				...
				try {
					sourceEntries.forEach(e => {
						...
						if (e[stat.targetPath]?.baseValue !== undefined) {
							if (baseSum === undefined) baseSum = 0;
							baseSum += Number(e[stat.targetPath]?.baseValue ?? 0);
						}
					});
				} catch (e) {
				...
				if (baseSum !== undefined) {
					...
					target[stat.targetPath] = { ...target[stat.targetPath], baseValue: Number(baseSum.toFixed(3)) };
				}
			}
	...
	updateJsonToday() {
		...
		try {
			...
			const today = {};
			const effectiveStats = this._effectiveStats();
			const todayBreakdownValues = this._breakdown.getTodayValues(); //NEU

			for (const stat of effectiveStats) {
				if (stat.type === statisticsType.computed) continue;
				const val = stat._isBreakdown ? todayBreakdownValues[stat.targetPath] : this.stateCache.get(stat.sourceId)?.value; //NEU
			...
			*/
			...
			// Pass 3: subtract breakdown values from consumption
			if (today.consumption?.value !== undefined) {
				today.consumption = { ...today.consumption, baseValue: today.consumption.value };
				let breakdownSum = 0;
				for (const stat of this._breakdown.statsEntries) {
					if (!stat._isBreakdown) continue;
	...
	_calculateLive() {
		...
		for (const stat of this._effectiveLiveStats()) {
			...
			*/
			...
			//NEU baseValue: If the baseValue is present, convert it to kW as well
			if (last[stat.targetPath].baseValue !== undefined) {
				const base = Math.round(last[stat.targetPath].baseValue * convFactor * 1000) / 1000;
				last[stat.targetPath] = { ...last[stat.targetPath], baseValue: base };
			}
		}
	...
	async mitNightProcess(calledFromInit = false) {
		const now = new Date();
		await this._executeScheduledTasks();
		if (!calledFromInit) {
			await this._breakdown.resetDayStart(); // capture new baseline for today's breakdown deltas
		}
	...
	async initialize() {
		...
		await this.mitNightProcess(true);
