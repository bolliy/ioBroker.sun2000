...
class statistics {
	...
	_calculateGeneric(stateId, periodStart, periodEnde, effectiveStats = this._effectiveStats()) {
		...
		if (entry.consumption?.value !== undefined) {
			entry.consumption = { ...entry.consumption, value2: entry.consumption.value };
			let breakdownSum = 0;
	...
	_calculateLive() {
		...
		for (const stat of this._effectiveLiveStats()) {
			...
			//NEU value2
			if (last[stat.targetPath].value2 !== undefined) {
				const kw2 = Math.round(last[stat.targetPath].value2 * convFactor * 1000) / 1000;
				last[stat.targetPath] = { ...last[stat.targetPath], value2: kw2 };
			}
		}
