			...
			if (stat.type === statisticsType.delta || stat.type === statisticsType.deltaReset) {
				if (last[stat.targetPath]?.total === undefined) {
					...
					if (stat.type === statisticsType.deltaReset) {
						// Handle resets: if the value dropped by more than 20% compared to the last total, assume a reset happened and don't subtract the last total (which would lead to a huge negative delta).
