...
class statistics {
	constructor(adapterInstance, stateCache) {
		...
		// [CACHE] Cache for the built-in (non-custom) chart template per chart type.
		// Key: `${chartStyle}|${showBat}|${showSOC}|${breakdownSignature}`.
		// Rebuilt only when the structural inputs change (battery presence, breakdown entries),
		// not on every scheduled chart tick.
		this._templateCache = new Map(); // myChart -> { key, template }
		...
		*/
		try {
			...
			if (Object.keys(parsed).length === 0) {
				template = this._getCachedDefaultTemplate(myChart, chartStyle);
			} else if (command === 'createTemplateFromBuiltin') {
				// Explicit user request to (re)generate — always bypass the cache.
				template = this._getCachedDefaultTemplate(myChart, chartStyle, true);
			} else {
				template = parsed;
				delete template._meta;
			}
	...
	// -------------------------------------------------------------------------
	// [CACHE] Default template caching
	// -------------------------------------------------------------------------

	/**
	 * Builds a signature string capturing everything that influences the
	 * structure* of the default template (legend entries, series composition,
	 * grid/axis layout). Data values themselves are NOT part of this signature —
	 * they are injected afterwards via placeholder string-replacement and don't
	 ...
	 * @returns {string}
	 */
	_templateCacheKey(myChart, chartStyle) {
		const showBat = this._batteryExists();
		const showSOC = showBat && (myChart === 'hourly' || myChart === 'live');
		// Breakdown entries affect legend + injected series (name/color/targetPath)
		const breakdownSig = this._breakdown.entries.map(e => `${e.targetPath}:${e.name}:${e.color}`).join('|');
		return `${chartStyle}|${showBat}|${showSOC}|${breakdownSig}`;
	}

	/**
	 * Returns the built-in default template for the given chart type, rebuilding
	 * it only when the structural signature has changed since the last build.
	 *
	 * @param {string} myChart
	 * @param {string} chartStyle
	 * @param {boolean} [forceRebuild] - bypass the cache (used for the explicit
	 *   'createTemplateFromBuiltin' command so a user-triggered regeneration
	 *   always produces a fresh template).
	 * @returns {object}
	 */
	_getCachedDefaultTemplate(myChart, chartStyle, forceRebuild = false) {
		const key = this._templateCacheKey(myChart, chartStyle);
		const cached = this._templateCache.get(myChart);

		if (!forceRebuild && cached && cached.key === key) {
			return cached.template;
		}

		const template = this._buildDefaultTemplate(myChart, chartStyle);
		this._templateCache.set(myChart, { key, template });
		this.adapter.logger.debug(`statistics: default template ${forceRebuild ? 'force-' : ''}rebuilt for ${myChart} (cache ${cached ? 'miss' : 'empty'})`);
		return template;
	}
	...
	*/
	...
	async handleBreakdownChange(state) {
		const changed = await this._breakdown.handleStateChange(state);
		if (!changed) return;
		this._templateCache.clear(); // [CACHE] structural change — force fresh templates
		for (const chartType of CHART_TYPES) {
			this._buildFlexchart(chartType);
		}
	}
	...
	destroy() {
		if (this.taskTimer) this.adapter.clearTimeout(this.taskTimer);
		if (this._liveTimer) this.adapter.clearTimeout(this._liveTimer);
		this._templateCache.clear(); // [CACHE]
	}
}
