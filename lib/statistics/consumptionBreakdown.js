...
const CACHE_KEY_PREFIX = 'statistics._bd.';

/** State ID for persisting the day-start baseline values (internal use, not user-editable) */
const DAYSTART_STATE_ID = 'statistics.consumptionBreakdownDayStart';
...
/**
 * Internal state that persists the cumulative source value of every breakdown
 * entry at the start of the day. Used by getTodayValues() to compute the
 * daily delta for statistics.jsonToday. Not meant to be edited by the user.
 */
const DAYSTART_STATE_DEF = {
	id: DAYSTART_STATE_ID,
	name: 'Consumption breakdown day-start baseline (internal)',
	type: 'string',
	role: 'json',
	desc: 'Internal: cumulative source values captured at the start of the day, used to compute today’s delta for each breakdown entry. Do not edit manually.',
	write: false,
	initVal: '{}',
};
...
class ConsumptionBreakdown {
	...
	constructor(adapterInstance, stateCache, builtinPaths) {
		...
		this._entries = [];
		this._dayStart = {}; // targetPath -> cumulative source value at start of day (persisted)
	}
	...
	/**
	 * The ioBroker state definitions that must be registered by statistics.js.
	 * @returns {object[]}
	 */
	get stateDefs() {
		return [BREAKDOWN_STATE_DEF, DAYSTART_STATE_DEF];
	}
	...
	async initialize() {
		...
		// load persisted day-start baseline (survives adapter restarts)
		const dayStartState = await this.adapter.getStateAsync(DAYSTART_STATE_ID);
		try {
			this._dayStart = JSON.parse(dayStartState?.val ?? '{}') || {};
		} catch {
			this._dayStart = {};
		}
		//!!NEU!!
		//this.stateCache.set(DAYSTART_STATE_ID, JSON.stringify(this._dayStart), { type: 'string', stored: true });
	}
	...
	/**
	 * Captures the current cumulative value of every breakdown entry as the new
	 * "day-start" baseline. Must be called once per day (shortly before midnight,
	 * right after refreshValues()), so getTodayValues() starts counting from 0
	 * for the new day. Persists the baseline so it survives adapter restarts.
	 *
	 * @returns {Promise<void>}
	 */
	async resetDayStart() {
		for (const bd of this._entries) {
			if (bd.type === statisticsType.level) continue; // no baseline needed for level values
			const current = this.stateCache.get(`${CACHE_KEY_PREFIX}${bd.targetPath}`)?.value;
			if (current != null) {
				this._dayStart[bd.targetPath] = current;
			}
		}
		//this.stateCache.set(DAYSTART_STATE_ID, JSON.stringify(this._dayStart), { type: 'string' });
		await this._persistDayStart();
	}

	/**
	 * Computes today's delta value for every active breakdown entry, using the
	 * day-start baseline captured by resetDayStart(). For 'deltaReset' entries a
	 * counter reset is detected the same way the periodic pipeline does (current
	 * value drops below 80% of the baseline) - in that case the raw current value
	 * is used as-is instead of going negative.
	 *
	 * If no baseline exists yet for an entry (e.g. right after it was added, or
	 * before the first midnight since adapter start), the current value is used
	 * as the baseline on the fly so today's value starts at 0.
	 *
	 * @returns {Record<string, number|null>} targetPath -> today's value (rounded, in `unit`)
	 */
	getTodayValues() {
		const result = {};
		let dirty = false;
		for (const bd of this._entries) {
			const current = this.stateCache.get(`${CACHE_KEY_PREFIX}${bd.targetPath}`)?.value;
			if (current == null) {
				result[bd.targetPath] = null;
				continue;
			}
			if (bd.type === statisticsType.level) {
				result[bd.targetPath] = Math.round((current + Number.EPSILON) * 1000) / 1000;
				continue;
			}
			let start = this._dayStart[bd.targetPath];
			if (start == null) {
				start = current;
				this._dayStart[bd.targetPath] = start;
				dirty = true;
			}
			let value = current - start;
			if (bd.type === statisticsType.deltaReset && current < start * 0.8) {
				value = current; // counter reset detected
			}
			if (value < 0) value = 0;
			result[bd.targetPath] = Math.round((value + Number.EPSILON) * 1000) / 1000;
		}
		if (dirty) {
			this._persistDayStart(); //fire and forget - only relevant for newly added entries
		}
		return result;
	}

	/** Persists the current day-start baseline map. */
	async _persistDayStart() {
		//this.stateCache.set(DAYSTART_STATE_ID, JSON.stringify(this._dayStart), { type: 'string' });
		try {
			await this.adapter.setState(DAYSTART_STATE_ID, { val: JSON.stringify(this._dayStart), ack: true });
		} catch (err) {
			this.adapter.logger.warn(`consumptionBreakdown: could not persist day-start baseline: ${err.message}`);
		}
	}
