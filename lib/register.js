...
class Registers {
	constructor(adapterInstance) {
		...
		this.postUpdateHooks = [
			{
				//deprecated! schoud be deactivated in the next version
				refresh : dataRefreshRate.low,
				state: {id: 'derived.dailyInputYield', name: 'Input Yield Today', type: 'number', unit: 'kWh', role: 'value.power.consumption', desc: 'deprecated! can be deactivated in the next version'},
				fn : (path) => {
					...
					const energyLoss = new Date().getHours()*this.inverterInfo.energyLoss;
					this.stateCache.set(path+'derived.dailyInputYield', inputYield - energyLoss, {type: 'number'});
				}
			}
		];
		this.postProcessHooks = [
			...
			},
			{
				...
				states : [
					...
					{id: 'collected.dailyInputYield', name: 'Input Yield Today', type: 'number', unit: 'kWh', role: 'value.power.consumption', desc: 'deprecated! can be deactivated in the next version'},
					{id: 'collected.dailySolarYield', name: 'Solar Yield Today', type: 'number', unit: 'kWh', role: 'value.power.consumption', desc: 'Riemann sum of input power with efficiency loss'},
	...
	wasAllRead(timeBefore) {
		...
		for (const inverter of this.adapter.inverters) {
			for (const [i, reg] of this.registerFields.entries()) {
				if (reg.states && reg.refresh) {
					...
					if (!lastread || now.getTime()-lastread > timeBefore) {
						if (!lastread && i == 0) {
							ret.errno = 101;
							ret.message = 'Can\'t read data from inverter! Please check the configuration.';
						} else {
							ret.errno = 102;
							ret.message =  'Not all data can be read. Please inspect the logs.';
						}
						return ret ;
					}
				}
		...
		return {errno: 0, message: 'No problems detected'};
	}
