		...
		this.postUpdateHooks = [
			{
				...
				fn : (path) => {
					...
					this.stateCache.set(path+'derived.dailyPortalYield', inputYield, {type: 'number'});
				}
			}
		];
		this.processHooks = [
			...
			},
			{
				...
				fn : (inverters) => {
					...
					for (const inverter of inverters) {
						...
						totalCharge += this.stateCache.get(inverter.path+'.battery.currentDayChargeCapacity')?.value;
						totalDisCharge += this.stateCache.get(inverter.path+'.battery.currentDayDischargeCapacity')?.value;
					...
					this.stateCache.set('collected.dailyPortalYield',inYield, {type: 'number'});
