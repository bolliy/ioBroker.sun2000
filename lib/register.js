		...
		this.registerFields = [
			...
			},
			{
				...
				states: [
					...
					{
						state: {id: 'string.PV1Voltage', name: 'PV1 voltage', type: 'number', unit: 'V', role: 'value.voltage'},
						register: {reg: 32016, type: dataType.int16, length: 1, gain: 10}
					},
					{
						state: {id: 'string.PV1Current', name: 'PV1 current', type: 'number', unit: 'A', role: 'value.current'},
				...
				postHook: (path) => {
					...
					if (noPVString) {
						this.adapter.log.debug('NoPVString is present ;)'+noPVString);
