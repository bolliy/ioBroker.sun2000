	...
	constructor(adapterInstance) {
		...
		this.registerFields = [
			...
			},
			{
				...
				{
					state: {id: 'derived.inputPowerEffectively', name: 'Input power Effectively', type: 'number', unit: 'kW', role: 'value.power', desc: 'Power from PV'}
				}
				],
				postHook: async (stateMap,path) => {
					...
					const inPower = stateMap.get('inputPower');
					const state = await this.adapter.getStateAsync(path+'info.ratedPower');
					//https://wiki.selfhtml.org/wiki/JavaScript/Operatoren/Optional_Chaining_Operator
					const ratedPower = state ? state.val : undefined;
					let inPowerEff = inPower?.value;
					...
					stateMap.set('derived.inputPowerEffectively', {id: path+'derived.inputPowerEffectively', value: inPowerEff});
				}
			},
			{
				address : 37113,
	...
	*/
			...
			},
			{
				...
				*/
				...
				{
					state: { id: 'battery.test', name: 'Current Day Discharge Capacity', type: 'string', unit: '', role: 'value', desc: '' }
				}],
				postHook: async (stateMap,path) => {
					stateMap.set('battery.test', {id: path+'battery.test', value: 'test'});
				}
			}
		];
		this.postUpdateHooks = [
			{
				...
				fn : async (stateMap, path) => {
					stateMap.set('derived.inputYieldDaily', {id: path+'derived.inputYieldDaily', value: 10});
				}
	...
	async storeStates(stateMap) {
		for (const stateEntry of stateMap.values()) {
			...
			if (stateEntry.value !== null && !stateEntry.stored ) { //&& !stateEntry.stored
			...
			for(const field of reg.states) {
				...
				if (field.register) {
					...
					if (value !== null) {
						...
						stateMap.set(state.id, {id: path+state.id, value: value});
					}
				}
			}
		}
	...
	async updateStates(modbusClient,refreshRate,duration) {
		...
		for (const reg of this.registerFields) {
			...
			try {
				...
				this.adapter.log.warn('err.code '+err.code);
				if (err.code == 'EHOSTUNREACH' || err.modbusCode == 6) break; //modbus is busy
			}
		}
		//Einschubfunktionen
		this.runPostUpdateHooks(refreshRate,this.stateMap);
