		...
		this.postProcessHooks = [
			{
				...
				fn: inverters => {
					...
					if (this.statistics.updateJsonToday && typeof this.statistics.updateJsonToday === 'function') {
						this.statistics.updateJsonToday();
					}
				},
			},
			{
				...
				fn: inverters => {
					...
					let charge = 0;
					let disCharge = 0;
					let statisticsCharge = 0;
					let statisticsDisCharge = 0;
					...
					for (const inverter of inverters) {
						...
						if (this.stateCache.get(`${inverter.path}.accumulatedEnergyYield`)?.value === undefined) {
							return; //wait until value is available - otherwise we would overwrite the value from the portal with 0
						}
						...
						activeEnergy += this.stateCache.get(`${inverter.path}.derived.dailyActiveEnergy`)?.value;
						...
						if (this.stateCache.get(`${inverter.path}.battery.ratedCapacity`)?.value > 0) {
							...
							statisticsCharge += this.stateCache.get(`${inverter.path}.battery.statistics.currentDayChargeCapacity`)?.value;
							statisticsDisCharge += this.stateCache.get(`${inverter.path}.battery.statistics.currentDayDischargeCapacity`)?.value;
					...
					if (sign === -1) {
						...
						feedinEnergy = this.stateCache.get('meter.reverseActiveEnergy')?.value;
						supplyFromGrid = this.stateCache.get('meter.positiveActiveEnergy')?.value;
					} else {
						feedinEnergy = this.stateCache.get('meter.positiveActiveEnergy')?.value;
						supplyFromGrid = this.stateCache.get('meter.reverseActiveEnergy')?.value;
					}
					//NEU - Initialisierung
					...
					this.stateCache.set('collected.statistics.currentDayChargeCapacity', statisticsCharge, { type: 'number' });
					this.stateCache.set('collected.currentDayDischargeCapacity', disCharge, { type: 'number' });
					this.stateCache.set('collected.statistics.currentDayDischargeCapacity', statisticsDisCharge, { type: 'number' });
