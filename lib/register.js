...
const {deviceType,deviceClasses,dataRefreshRate} = require(__dirname + '/types.js');
...
class Registers {
	constructor(adapterInstance) {
		...
		for (const device of this.adapter.devices) {
			//DriverInfo Instance
			const handler = getDriverHandler(this.adapter.settings.deviceClass);
			if (handler) {
		...
		//only Inverter
		if (this.adapter.settings.deviceClass == deviceClasses.inverter) {
