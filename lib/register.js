...
class Registers {
	...
	async updateStates(inverter,modbusClient,refreshRate,duration) {
		...
		if (inverter.modelId && inverter.device.info?.modelId !== inverter?.modelId) {
			this.createInverterInstance(inverter,inverter.modelId);
			//this.adapter.log.debug(JSON.stringify(inverter.device.info));
			if (inverter.device.info?.modelId !== inverter.modelId) {
				if (inverter.modelId >= 0) {
					this.adapter.log.error('No driver could be assigned for model id '+inverter.modelId);
				...
				//throw new Error('Parameter is not a number!');; //Error
				return -1;
			}
		}
