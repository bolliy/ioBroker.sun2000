			...
			for (const [i, reg] of this.registerFields.entries()) {
				if (reg.states && reg.refresh) {
					...
					if (!lastread) {
						if (i==0) {
							ret.errno = 1;
							ret.message = 'Can\'t read any data from inverter ID '+inverter.modbusId;
						} else {
							ret.errno = 2;
							ret.message =  'Not all data can be read by the inverter ID '+inverter.modbusId;
						}
						return ret;
					}
					if (now.getTime()-time > lastread) {
						ret.errno = 3;
						ret.message = 'Not all data can be read! Please note the other warning messages.';
						return ret ;
					}
				}
			}
