		...
		this.inverterPostProcessHooks = [
			{
				...
				states: [
					...
					{ id: 'collected.gridExportPower', name: 'Grid Export power', type: 'number', unit: 'kW', role: 'value.power', desc: 'Power to grid' },
				...
				fn: inverters => {
					...
					let feedinPower = this.stateCache.get('meter.activePower')?.value ?? 0;
					if (this.adapter.settings.integration === 2) {
						//Emma Meter Power is positive when power is taken from grid
						//Emma Meter Power is negative when power is feed in to grid
						//=> we need to invert the value here to get the same behavior as with other meters
					...
					this.stateCache.set('collected.gridExportPower', feedinPower, { type: 'number' }); //positive -> power to grid
			...
			},
			{
				...
				fn: inverters => {
					...
					if (this.adapter.settings.integration === 2) {
						...
						feedoutEnergy = this.stateCache.get('meter.activeActiveEnergy')?.value ?? 0;
					} else {
						feedinEnergy = this.stateCache.get('meter.positiveActiveEnergy')?.value ?? 0;
						feedoutEnergy = this.stateCache.get('meter.reverseActiveEnergy')?.value ?? 0;
					}
					const conSum = enYield + feedoutEnergy - feedinEnergy;
					this.stateCache.set('collected.consumptionSum', conSum, { type: 'number' });
					// compute export and import today
					this.stateCache.set('collected.gridExportToday', feedoutEnergy - this.stateCache.get('collected.gridExportStart')?.value, {
						type: 'number',
					});
					this.stateCache.set('collected.gridImportToday', feedinEnergy - this.stateCache.get('collected.gridImportStart')?.value, {
						type: 'number',
					});
					this.stateCache.set(
