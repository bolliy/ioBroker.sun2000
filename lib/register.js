		...
		this.inverterPostProcessHooks = [
			{
				...
				states: [
					...
					{ id: 'collected.feed-inPower', name: 'feed-in power', type: 'number', unit: 'kW', role: 'value.power', desc: 'Power to grid' },
				...
				fn: inverters => {
					...
					const directionOfPowerFlow = this.stateCache.get('meter.derived.directionOfPowerFlow')?.value ?? 0;
					if (directionOfPowerFlow === 1) {
			...
			},
			{
				...
				fn: inverters => {
					...
					this.stateCache.set('collected.gridImportToday', feedoutEnergy - this.stateCache.get('collected.gridImportStart')?.value, {
						type: 'number',
					});
					this.stateCache.set(
