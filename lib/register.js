...
const {InverterInfo, getInverterHandler} = require(__dirname + '/inverter.js');
...
class Registers {
	constructor(adapterInstance) {
		...
		for (const inverter of this.adapter.inverters) {
			inverter.device = new InverterInfo(this,inverter);
		}
	...
	async updateStates(inverter,modbusClient,refreshRate,duration) {
		...
		if (inverter.modelId && inverter.device?.modelId !== inverter?.modelId) {
			const inverterHandler = getInverterHandler(inverter.modelId);
			if (inverterHandler) inverter.device = new inverterHandler(this,inverter);
		}
