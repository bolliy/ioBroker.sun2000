		...
		this.postProcessHooks = [
			...
			},
			{
				...
				states: [
					...
					{ id: 'collected.consumptionStart', name: 'Consumption start today', type: 'number', unit: 'kWh', role: 'value.power.consumed' },
				...
				fn: inverters => {
					...
					for (const inverter of inverters) {
						...
						if (this.stateCache.get(`${inverter.path}.battery.ratedCapacity`)?.value > 0) {
							...
							/*
							if (this.stateCache.get(`${inverter.path}.battery.statistics.currentDayChargeCapacity`)?.value === undefined) {
								
								charge += this.stateCache.get(`${inverter.path}.battery.currentDayChargeCapacity`)?.value ?? 0;
							} else {
								charge += this.stateCache.get(`${inverter.path}.battery.statistics.currentDayChargeCapacity`)?.value;
							}
							if (this.stateCache.get(`${inverter.path}.battery.statistics.currentDayDischargeCapacity`)?.value === undefined) {
								disCharge += this.stateCache.get(`${inverter.path}.battery.currentDayDischargeCapacity`)?.value ?? 0;
							} else {
								disCharge += this.stateCache.get(`${inverter.path}.battery.statistics.currentDayDischargeCapacity`)?.value;
							}
							*/
