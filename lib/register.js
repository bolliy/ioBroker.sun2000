...
class Registers {
	constructor(adapterInstance) {
		...
		/*
		// Hook: hourly statistics for house consumption
		this.postProcessHooks.push({
			refresh: dataRefreshRate.low,
			states: [
				{
					id: 'statistik.consumption.jsonHourly',
			...
			fn: () => {
				try {
					const now = new Date();
					// only run at full hour
					if (now.getMinutes() !== 0) return;
					const prevHour = new Date(now);
					prevHour.setHours(prevHour.getHours() - 1, 0, 0, 0);

					const consumption = this.stateCache.get('collected.houseConsumption')?.value;
					...
					const timeStr = localIsoWithOffset(prevHour);
					let jsonStr = this.stateCache.get('statistik.consumption.jsonHourly')?.value ?? '[]';
					let arr = [];
					try {
					...
					arr.push({ time: timeStr, consumption: Number(value.toFixed(3)) });
					...
					this.stateCache.set('statistik.consumption.jsonHourly', JSON.stringify(arr), { type: 'string' });
					this.adapter.logger.debug(`Appended hourly statistic ${timeStr} val=${value}`);
				} catch (err) {
	...
	async _loadStates() {
		...
		state = await this.adapter.getState('statistik.consumption.jsonHourly');
		this.stateCache.set('statistik.consumption.jsonHourly', state?.val ?? '[]', { type: 'string', stored: true });
		*/
	}
