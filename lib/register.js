...
const {deviceType,storeType,driverClasses,dataRefreshRate} = require(__dirname + '/types.js');
...
class Registers {
		...
		for (const stateEntry of this.stateCache.values()) {
			...
			if (stateEntry.value !== null) {
				try {
					...
					this.adapter.log.debug(`Fetched ${stateEntry.id}, val=${stateEntry.value}, hook=${hook} `);
				} catch (err) {
	...
	async runPostProcessHooks(refreshRate) {
		...
		this.storeStates(true); //fire and forget
	}
	...
	CheckReadError(timeShift) {
		...
		for (const device of this.adapter.devices) {
			//v0.4.x
			if (!device.instance.info.modbusAllowed) continue;
