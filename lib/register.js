		...
		this.inverterPostProcessHooks = [
			...
			},
			{
				...
				states: [
					...
					{ id: 'collected.dailyActiveEnergy', name: 'Active Energy today', type: 'number', unit: 'kWh', role: 'value.power.consumption', desc: '' },
				],
				fn: inverters => {
					let inYield = 0;
					let solarYield = 0;
					...
					for (const inverter of inverters) {
						...
						inYield += this.stateCache.get(`${inverter.path}.derived.dailyInputYield`)?.value; //input yield
					...
					*/
					//consumtionToday =  activeEnergy - collected.gridImportToday + collected.gridExportToday
					this.stateCache.set(
						'collected.consumptionToday',
