...
class Registers {
	constructor(adapterInstance) {
		...
		this.registerFields = [
			...
			},
			{
				...
				{
					state: {id: 'derived.dailySolarYield', name: 'Solar Yield Today', type: 'number', unit: 'kWh', role: 'value.power.consumption', desc: 'Riemann sum of input power'}
				}
				],
				postHook: (path) => {
					...
					this.inverterInfo.solarSum.add(inPower); //riemannSum
	...
	async _loadStates() {
		...
		for (const inverter of this.adapter.inverters) {
			state = await this.adapter.getStateAsync(inverter.path+'.derived.sdailySolarYield');
