		...
		if (this._initialized) {
			...
			for (const event of this._eventMap.values()) {
				...
				if (event.value !== null && service.fn) {
					...
					if (event.type == deviceType.battery) {
						const BatStatus = this.inverterInfo.instance.stateCache.get(this.inverterInfo.path+'.battery.runningStatus')?.value;
						//
						if (BatStatus !== 'RUNNING' || BatStatus !== 'STANDBY') {
							this.log.info(`Control: Event is discarded because battery is not running. ${this.inverterInfo.path}.battery.runningStatus: '${BatStatus}. `);
