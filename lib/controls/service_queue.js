...
class ServiceQueueMap  {
	constructor (adapterInstance,inverter) {
		...
		this.serviceFields = [
			...
			{
				state: {id: 'activePower.activePowerControlMode', name: 'Active Power Control Mode', type: 'number', unit: '', role: 'value', desc: 'reg:47415, len:1'},
			...
			{
				state: { id: 'activePower.maximumFeedGridPower', name: 'Maximum Feed Grid Power', type: 'number', unit: 'kW', role: 'value.power', desc: 'reg: 47416, len: 2'},
				type : deviceType.powerControl,
				fn: async event => {
					const max = 1000;
					if (event.value > max) event.value = max;
			...
			{
				state: { id: 'activePower.maximumFeedGridPower_percent', name: 'Maximum Feed Grid Power %', type: 'number', unit: '%', role: 'value', desc: 'reg: 47418, len: 1'},
				type : deviceType.powerControl,
				fn: async event => {
					if (event.value > 100) event.value = 100;
					if (event.value < 0) event.value = 0;
	...
	async _init() {
		if (this.inverterInfo.instance) {
			for (const item of this.serviceFields) {
				...
				if (item.type == deviceType.powerControl) {
					if (!this.adapter.settings.powerControl || !this.inverterInfo?.meter) continue; //v0.8.0
				}
				if (item?.state) {
