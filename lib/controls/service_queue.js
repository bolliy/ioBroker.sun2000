...
class ServiceQueueMap  {
			...
			if (this.inverterInfo?.numberBatteryUnits > 0 && this._TOU) {
				...
				if (workingMode &&  workingMode[0] !== 5 ) {
					...
					*/
					...
					if (await this._writeRegisters(47255,tCDP)) this.adapter.log.info('Control: The default TOU settings are stored.');
				}
			}
		}
	}
	...
	set(id, state) {
		...
		if (state && service) {
			this.adapter.log.info(`Control: Event - state: ${id} changed: ${state.val} ack: ${state.ack}`);
				...
				if (event.value !== null && service.fn) {
					...
					if (await service.fn(event)) {
						try {
							...
							this.adapter.log.info('Control: write state '+this.inverterInfo.path+'.control.'+event.id+' : '+event.value+' ack: true');
						} catch {
							this.adapter.log.warn('Control: Can not write state '+this.inverterInfo.path+'.control.'+event.id);
						}
					}
				}
	...
	async _writeRegisters(address,data) {
		try {
			this.adapter.log.debug('Try to write data to id/address ' + this._modbusClient.id + '/' + address+'/'+data.length);
			await this._modbusClient.writeRegisters(address,data);
			this.inverterInfo.instance.addHoldingRegisters(address,data);
			return true;
		} catch (err) {
			this.adapter.log.warn(`Error while writing to ${this._modbusClient.ipAddress} [Reg: ${address}, Len: ${data.length}, modbusID: ${this._modbusClient.id}] with: ${err.message}`);
		}
	}

	async _readHoldingRegisters(address,length) {
		try {
			this.adapter.log.debug('Try to read data to id/address ' + this._modbusClient.id + '/' + address+'/'+length);
			const data = await this._modbusClient.readHoldingRegisters(address,length);
			return data;
		} catch (err) {
			this.adapter.log.warn(`Error while reading from ${this._modbusClient.ipAddress} [Reg: ${address}, Len: ${length}, modbusID: ${this._modbusClient.id}] with: ${err.message}`);
		}
	}
