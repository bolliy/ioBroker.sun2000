...
class ServiceQueueMap  {
	constructor (adapterInstance,inverter) {
		...
		this.serviceFields = [
			...
			{
				state: { id: 'battery.ForcibleChargeOrDischargeSettingMode', name: 'Forcible charge/discharge setting mode (0: Duration)', type: 'number', unit: '', role: 'value', desc: 'reg: 47246, len: 1'},
				type : deviceType.battery,
				fn: async event => {
					if (event.value > 65535) event.value = 65535;
					if (event.value < 0) event.value = 0;
