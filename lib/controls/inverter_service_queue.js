...
class ServiceQueueMap {
	...
	async _init() {
		...
		if (this.inverterInfo.instance) {
			...
			for (const entry of this._serviceMap.values()) {
				...
				if (emmaAvailable) {
					await this._initState(path, entry.state, entry.state.readOnlyIfEmmaAvailable === true ? true : false);
				} else {
	...
	async _delState(id) {
		try {
			await this.adapter.delObject(id, { recursive: false });
		} catch {
			this.log.warn(`${this._name}: Can not delete state ${id}`);
		}
	}

	//state
	async _initState(path, state, reaOnly = false) {
