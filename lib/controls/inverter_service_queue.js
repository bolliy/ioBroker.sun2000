...
const { touJsonToRegisters } = require(`${__dirname}/utils.js`);
...
class ServiceQueueMap {
	constructor(adapterInstance, device) {
		...
		this._name = `inverter ${this.inverterInfo?.index} control`;
		...
		this.serviceFields = [
			...
			*/
			...
			//TOU Charge/Discharge time windows (user-friendly JSON)
			{
				state: {
					...
					role: 'json',
					desc:
						'reg: 47255, len: 43 | JSON array, max 14 segments. ' +
				...
				fn: async (event, service) => {
					...
					const ret = await this._writeRegisters(47255, words);
					if (ret) {
		...
		if (this.inverterInfo.instance) {
			...
			if (this.adapter.settings?.cb.tou && this.inverterInfo.instance.numberBatteryUnits() > 0) {
				...
				if (workingMode && workingMode[0] !== 5) {
					const touStateId = `${this.inverterInfo.path}.control.battery.tou.chargeDischargeTimeWindows`;
					...
					if (!existing || existing.val === null || existing.val === '' || existing.val === '[]') {
						try {
							...
							if (await this._writeRegisters(47255, words)) {
			...
			/*
			try {
				const touData = await this._readHoldingRegisters(47255, 43);
				if (touData && touData.length >= 43) {
					...
					if (!existing || existing?.val === null) {
						...
						this.log.debug(`${this._name}: TOU time windows read from device and stored in state`);
					}
				}
			} catch (err) {
				...
				if (event.value !== null && service.fn) {
					...
					if (await service.fn(event, service)) {
						...
						if (service.errorCount > 1) {
							...
							this.log.warn(`${this._name}: Event is discarded because it could not be processed. ${this.inverterInfo.path}.control.${event.id}`);
						}
					}
				}
				if (count > 1) {
