...
const { touJsonToRegisters, touRegistersToJson } = require(`${__dirname}/utils.js`);
...
class ServiceQueueMap {
	constructor(adapterInstance, emma) {
		...
		this.serviceFields = [
			...
			// --- reg 40004: TOU Charge/Discharge time windows (user-friendly JSON) ---
			{
				state: {
					id: 'battery.tou.chargeDischargeTimeWindows',
					name: '[Time of Use mode] Charge/discharge time windows',
					type: 'string',
					unit: '',
					role: 'json',
					desc:
						'reg: 40004, len: 43 | JSON array, max 14 segments. ' +
						'Each: {"start":"HH:MM","end":"HH:MM","mode":"charge|discharge","days":["Mon","Tue",...]}. ' +
						'Days: Mon Tue Wed Thu Fri Sat Sun',
				},
				type: deviceType.battery,
				fn: async (event, service) => {
					let words;
					try {
						words = touJsonToRegisters(event.value);
					} catch (err) {
						//only one warning for the same error
						if (service.errorCount === 0) {
							this.log.warn(`${this._name}: TOU time window invalid – ${err.message}`);
						}
						return false;
					}
					const ret = await this._writeRegisters(40004, words);
					if (ret) {
						this.log.debug(`${this._name}: TOU time windows written (${words[0]} segment(s))`);
					}
					return ret;
				},
			},
			// --- Issue #285: Limited feed-in / power control at the grid connection point ---
			{
				state: {
					id: 'grid.activePowerControlMode',
					name: '[Limited feed-in] Power control mode at grid connection point (0: unlimited, 5: zero power, 6: limited feed-in (kW), 7: limited feed-in (%))',
					type: 'number',
					unit: '',
					role: 'level',
					desc: 'reg: 40100, len: 1',
				},
				type: deviceType.gridPowerControl,
				fn: async event => {
					const allowed = [0, 5, 6, 7];
					if (!allowed.includes(event.value)) {
						event.value = 0;
					}
					const ret = await this._writeRegisters(40100, dataType.numToArray(event.value, dataType.uint16));
					return ret;
				},
			},
			{
				state: {
					id: 'grid.limitationMode',
					name: '[Limited feed-in] Limitation mode (0: total power, 1: single-phase power)',
					type: 'number',
					unit: '',
					role: 'level',
					desc: 'reg: 40101, len: 1',
				},
				type: deviceType.gridPowerControl,
				fn: async event => {
					...
					const ret = await this._writeRegisters(40101, dataType.numToArray(event.value, dataType.uint16));
					if (ret) {
						this.inverterInfo.instance.stateCache.set(`emma.${event.id}`, event.value, { type: 'number' });
					}
					return ret;
			...
			},
			{
				state: {
					...
					name: '[Limited feed-in] Maximum grid feed-in power',
					type: 'number',
					unit: 'kW',
					role: 'level.power',
					desc: 'reg: 40107, len: 2',
				},
				type: deviceType.gridPowerControl,
				fn: async event => {
					if (event.value > 5000) {
					...
					const ret = await this._writeRegisters(40107, dataType.numToArray(event.value * 1000, dataType.int32));
			...
			},
			{
				...
				fn: async event => {
					...
					const ret = await this._writeRegisters(40109, dataType.numToArray(event.value * 10, dataType.uint16));
			...
			{
				state: {
					id: 'grid.threePhaseImbalanceControl',
					name: '[Limited feed-in] Three-phase imbalance control (0: disabled, 1: enabled)',
					type: 'boolean',
					unit: '',
					role: 'switch.enable',
					desc: 'reg: 40110, len: 1',
				},
				type: deviceType.gridPowerControl,
				fn: async event => {
					const ret = await this._writeRegisters(40110, event.value === true ? [1] : [0]);
					/*
					if (ret) {
						this.inverterInfo.instance.stateCache.set(`emma.${event.id}`, event.value === true ? 1 : 0);
					}
					*/
					return ret;
				},
			},
			// --- Issue #285: SmartGuard ---
			{
				state: {
					id: 'smartGuard.powerSupplyConfiguration',
					name: '[SmartGuard] Power supply configuration (0: none, 1: mains only, 2: mains + generator, 3: generator only)',
					type: 'number',
					unit: '',
					role: 'level',
					desc: 'reg: 41214, len: 1',
				},
				fn: async event => {
					...
					const ret = await this._writeRegisters(41214, dataType.numToArray(event.value, dataType.uint16));
					return ret;
				},
			},
			{
				state: {
					id: 'smartGuard.considerMainsFaultyIf',
					name: '[SmartGuard] Consider mains to be faulty if (0: open, 1: closed)',
					type: 'boolean',
					unit: '',
					role: 'switch.enable',
					desc: 'reg: 41215, len: 1',
				},
				fn: async event => {
					const ret = await this._writeRegisters(41215, event.value === true ? [1] : [0]);
	...
	async _init() {
		if (this.inverterInfo.instance) {
			...
			// Pre-populate TOU state by reading current register values from the device
			/*
			try {
				const touData = await this._readHoldingRegisters(40004, 43);
				if (touData && touData.length >= 43) {
					const jsonStr = touRegistersToJson(touData);
					const touStateId = 'emma.control.battery.tou.chargeDischargeTimeWindows';
					const existing = await this.adapter.getState(touStateId);
					// Only write back if state is empty / never set (avoid overwriting a pending user edit)
					if (!existing || existing.val === null || existing.val === '' || existing.val === '[]') {
						await this.adapter.setState(touStateId, { val: jsonStr, ack: true });
						this.log.info(`${this._name}: TOU time windows read from device and stored in state`);
					}
				}
			} catch (err) {
				this.log.debug(`${this._name}: Could not read TOU registers on init: ${err.message}`);
			}
			*/

			if (this.adapter.settings?.cb.tou) {
				const essControlMode = await this._readHoldingRegisters(40000, 1);
				if (essControlMode && essControlMode[0] !== 5) {
					const touStateId = 'emma.control.battery.tou.chargeDischargeTimeWindows';
					const existing = await this.adapter.getState(touStateId);
					// Only write default tou if state is empty / never set (avoid overwriting a pending user edit)
					if (!existing || existing.val === null || existing.val === '' || existing.val === '[]') {
						try {
							const words = touJsonToRegisters([
								{
									start: '00:00',
									end: '23:59',
									mode: 'charge',
							...
							if (await this._writeRegisters(40004, words)) {
								this.inverterInfo.instance.addHoldingRegisters(40004, words);
								this.log.info(`${this._name}: The default TOU setting are transferred`);
							}
						} catch (err) {
							this.log.warn(`${this._name}: TOU time window invalid for default setting – ${err.message}`);
						}
					}
				}
			}
		...
		if (this._initialized) {
			...
			for (const event of this._eventMap.values()) {
				...
				if (event.value !== null && service.fn) {
					...
					count++;
					if (await service.fn(event, service)) {
						service.errorCount = 0;
						try {
							event.ack = true;
						...
						if (service.errorCount > 1) {
							...
							this.log.warn(`${this._name}: Event is discarded because it could not be processed. State: emma.control.${event.id}`);
						}
					}
				}
				if (count > 1) {
...
module.exports = { ServiceQueueMap, touJsonToRegisters, touRegistersToJson };
