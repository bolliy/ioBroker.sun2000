...
class Logging {
	...
	debug(msgFn) {
		if (this.adapter.log.level === 'debug' || this.adapter.log.level === 'silly') {
			this.adapter.log.debug(typeof msgFn === 'function' ? msgFn() : msgFn);
		}
	}
...
class RegisterMap {
	constructor() {
		this._map = new Map();
		this._tsMap = new Map(); //timestamp of last write per address - used by the modbus-proxy to detect stale cache entries
	...
	set(startAddr, values) {
		...
		for (let i = 0; i < values?.length; i++) {
			...
			this._tsMap.set(startAddr + i, now); //NEW
		}
	}

	/**
	 * Returns the timestamp of the least recently updated address within the
	 * given range, or undefined if any address in the range has never been written.
	 * Used to determine whether a cached register block is stale.
	 *
	 * @param {number} startAddr
	 * @param {number} length
	 * @returns {number|undefined}
	 */
	getOldestTimestamp(startAddr, length) {
		let oldest;
		for (let i = 0; i < length; i++) {
			const ts = this._tsMap.get(startAddr + i);
			if (ts === undefined) {
				return undefined; //at least one word was never written
			}
