...
async function initState(adapter, path, state) {
	//this.adapter.log.debug('[_initStat] path+id '+path+state.id);
	await adapter.extendObject(path + state.id, {
		type: 'state',
		common: {
			name: state.name,
			type: state.type,
	...
	if (state.initVal) {
		const ret = await adapter.getState(state.id);
		if (!ret || ret.val === null) {
			try {
				await adapter.setState(state.id, { val: state.initVal, ack: true });
			} catch (err) {
				adapter.log.warn(`Error while initializing ${state.id}, val=${state.initVal} err=${err.message}`);
			}
		}
	}
}
