...
const {RegisterMap} = require(__dirname + '/../tools.js');
...
class DriverBase {
	...
	async updateStates(modbusClient,refreshRate,duration) {
		...
		//v1.x
		if (this.service && refreshRate === dataRefreshRate.high) {
			await this.service.process(modbusClient);
		}
