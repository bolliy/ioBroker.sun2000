...
class DriverBase {
	...
	async updateStates(modbusClient,refreshRate,duration) {
		...
		for (const reg of this.registerFields) {
			...
			if (reg.type == deviceType.battery && this.deviceInfo?.battery.unit1.exist === false && this.deviceInfo?.battery.unit2.exist === false) continue; //battery exist
			if (reg.type == deviceType.batteryUnit2 && this.deviceInfo?.battery.unit2.exist === false) continue; //battery Unit2
			*/
			if (reg.type == deviceType.gridPowerControl && !this.deviceInfo?.meter) continue; //Grid Power Control - v0.8.x
			/*
			if (reg.checkIfActive) {
				if (reg.checkIfActive(this._getStatePath(reg.type))) {
					this.log.debug('### checkIfActive: TRUE ###');
				} else {
					this.log.debug('### checkIfActive: FALSE FALSE ###');
				}
			}
			*/
