...
class DriverBase {
	...
	/**
	 * Determines whether the cached register data for the given address range
	 * is stale, based on the age of the cached values compared to the refresh
	 * interval of the matching registerFields entry (if any).
	 *
	 * Used by the Modbus-proxy (_handleGetReg) to decide whether a direct read
	 * from the device is necessary instead of serving the cached value - this
	 * also covers the case where a value was previously written via the proxy
	 * (setRegister/setRegisterArray) and the device may have changed it since.
	 *
	 * @param {number} startAddr
	 * @param {number} length
	 * @returns {boolean} true if the cache is stale or was never populated, false if still fresh
	 */
	isRegisterStale(startAddr, length) {
		const ts = this._regMap.getOldestTimestamp(startAddr, length);
		if (ts === undefined) {
			return true; //never cached - treat as stale so the proxy attempts a direct read
		}

		//try to find the matching registerFields entry to use its actual refresh rate
		const reg = this.registerFields.find(r => r.address === startAddr || (startAddr >= r.address && startAddr + length <= r.address + r.length));
		/*
		let interval = this.adapter.settings.lowInterval ?? 60000;
		if (reg?.refresh === dataRefreshRate.medium) {
			interval = this.adapter.settings.mediumInterval ?? 30000;
		} else if (reg?.refresh === dataRefreshRate.high) {
			//there is no explicit "high" interval setting - high refresh registers are polled continuously,
			//so a short threshold is enough to catch cases where polling has stalled (e.g. standby/error)
			interval = this.adapter.settings.highInterval ?? 5000;
		}
		*/
		if (reg) {
			return false;
		}
		const interval = this.adapter.settings.mediumInterval ?? 30000;
