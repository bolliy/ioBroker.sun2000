...
class DriverBase {
	...
	//v0.8.x
	_checkValidValueRange(value, type) {
		if (typeof value === 'number') {
			if (value === 0) return value; //is always correct
			...
			this.log.warn('_checkValidValueRange '+value+' smallest: '+smallest+' biggest: '+biggest);
			return 0;
		}
		return value;
	}
		...
		if (reg.states) {
			for(const field of reg.states) {
				...
				if (field.register) {
					...
					if (value !== null) {
						//v0.8.x
						if (field.register.type) value = this._checkValidValueRange(value,field.register.type);
	...
	async updateStates(modbusClient,refreshRate,duration) {
		...
		for (const reg of this.registerFields) {
			...
			if (reg.type == deviceType.powerControl && !this.deviceInfo?.powerControl) continue; //power control v0.8.x
