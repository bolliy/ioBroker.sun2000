...
const {RegisterMap,RiemannSum} = require(__dirname + '/../tools.js');
		...
		for (const reg of this.registerFields) {
			if (duration) {
				if (this._newNowTime() - start > (duration - this.adapter.settings.modbusDelay)) {
					this.adapter.log.debug('### Duration: '+Math.round(duration/1000)+' used time: '+ (this._now.getTime() - start)/1000);
					break;
				}
			}
			//v0.4.x
			...
			try {
				this.adapter.log.debug('Try to read data from id/address ' + modbusClient.id + '/' + reg.address+'/'+reg.refresh+'/'+reg.info);
	...
	constructor (stateInstance,device) {
		...
		const newFields = [
			...
			},
			{
				address : 65521,
				length : 14,
				info : 'smart logger public (indicator)',
				type : deviceType.meter,
				states: [{
					state: {id: 'logger.name', name: 'smart logger name', type: 'string', role: 'info.name', desc: 'reg:65524, len:10'},
					register: {reg: 65524 , type: dataType.string, length: 10}}
				],
				readErrorHook: (reg) => {
					//modbus Error 2 - illegal address
					reg.lastread = this._newNowTime();
					this.adapter.log.debug('No Huawei smartLogger could be identified !');
				},
				postHook: () => {
					const path = this._getStatePath(deviceType.meter);
					const detectedName = this.stateCache.get(path+'logger.name')?.value;
					if (detectedName) {
						this._newInstance = new SmartLogger(this.state,device);
						this.adapter.log.debug('Huawei smartLogger could be identified.');
					} else {
						this.adapter.log.debug('No Huawei smartLogger could be identified !');
					}
				}
		...
		const newHooks = [
			{
				...
				fn : () => {
					//this.adapter.log.debug('#### Hook Path '+path);
					if (!this._newInstance) {
						this.adapter.log.warn('No Huawei device could be identified!');
						this._newInstance = new SmartLogger(this.state,device);
					}
				}
			}
	...
	constructor(stateInstance,charger,options) {
		...
		const newFields = [
			...
			},
			{
				...
				refresh : dataRefreshRate.slow,
				states : [{
					state: {id: 'sdongle.deviceSearchStatus', name: 'Device Search Status', type: 'number', unit: '', role: 'value', desc: 'reg:37411, len:1'},
			...
			},
			{
				address : 35122,
				length : 1,
				info : 'SDongle Traffic Status',
				refresh : dataRefreshRate.slow,
				states : [{
					state: {id: 'sdongle.trafficStatus', name: 'Traffic Status', type: 'number', unit: '', role: 'value', desc: 'reg:35122, len:1'},
					register: {reg: 35122, type: dataType.uint16}
				}]
			},
			{
				address : 37498,
				...
				refresh : dataRefreshRate.medium,
				states : [{
					state: {id: 'sdongle.totalInputPower', name: 'Total Input Power', type: 'number', unit: 'kW', role: 'value.power', desc: 'reg:37498, len:2'},
				...
				{
					state: {id: 'sdongle.totalBatteryPower', name: 'Total Battery Power', type: 'number', unit: 'kW', role: 'value.power', desc: 'reg:37504, len:2'},
					register: {reg: 37504, type: dataType.int32, gain: 1000}
				},
				{
					state: {id: 'sdongle.totalActivePower', name: 'Total Active Power', type: 'number', unit: 'kW', role: 'value.power', desc: 'reg:37516, len:2'},
