...
const ServiceQueueMap = require(__dirname + '/../services/service_queue.js');
		...
		const newFields = [
			{
				...
				readErrorHook: (reg) => {
					...
					this.adapter.log.error('Modbus Error 2: Huawei inverter could not be identified for modbus ID '+this._modbusId+'!');
				},
				postHook: (path) => {
					...
					if (detectedModelId) {
						...
						this.adapter.log.error('Huawei inverter could not be identified for modbus ID '+this._modbusId+'!');
					}
				}
			}
		];
		...
		const newHooks = [
			{
				fn : () => {
					if (!this._newInstance) {
						this.adapter.log.warn('Huawei inverter could not be identified for modbus ID '+this._modbusId+'!');
					}
				}
			}
		];
...
class InverterSun2000 extends DriverBase{
	constructor(stateInstance,inverter,options) {
		...
		//v1.x
		this.service = new ServiceQueueMap(this.adapter,this.deviceInfo);
