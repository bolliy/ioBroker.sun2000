		...
		const newHooks = [
			{
				...
				fn: path => {
					...
					if (Math.abs(activePower - inputPower + chargePower) > 5) {
						const newValue = Math.round((inputPower - chargePower) * 1000) / 1000;
						this.log.debug(`activePower ${activePower} !== inputPower ${inputPower}-chargePower ${chargePower} (${newValue})`);
						this.stateCache.set(`${path}activePower`, newValue, { type: 'number' });
						/*
						if (chargePower === 0) {
							this.stateCache.set(`${path}activePower`, newValue, { type: 'number' });
						} else {
							const newValue = inputPower - activePower;
							this.stateCache.set(`${path}battery.chargeDischargePower`, newValue, { type: 'number' });
						}
						*/
					}
				},
