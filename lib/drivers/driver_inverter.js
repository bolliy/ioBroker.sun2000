		...
		const newFields = [
			...
			},
			{
				...
				states: [{
					...
					mapper: async value => {
						if (this._testMode) {
							this.log.info('testMode: Scheint die Sonne? '+isSunshine(this.adapter));
				...
				readErrorHook: (err) => { //(err,reg)
					if (err.modbusCode === undefined) {
						//inverter nicht erreichbar - ggf deviceStatus setzen
						//this.stateCache.set(this._getStatePath()+'deviceStatus', 2, {type: 'number'});
						this.log.info('deviceStatus cannot be read! '+err?.message);
		...
		if (modbusAllowed) {
			...
			if (this.deviceStatus == 0x0002) { //detecting irradiation
				if (this.adapter.settings.sunrise) {
					...
					modbusAllowed = timeAfterSunrise > 0 && timeAfterSunrise < 60*60*1000; //60 Minutes after sunrise
				}
