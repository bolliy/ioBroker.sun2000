		...
		const newHooks = [
			...
			},
			{
				...
				fn: path => {
					...
					let activeEnergy = this.stateCache.get(`${path}derived.dailyActiveEnergy`)?.value ?? 0;
					activeEnergy = activeEnergy + charge - disCharge; //activeEnergy * 0.97 + charge - disCharge;
					if (activeEnergy < 0) {
						activeEnergy = 0;
					}
					this.stateCache.set(`${path}derived.dailyInputYield`, activeEnergy, { type: 'number' });
				},
			},
			{
				refresh: dataRefreshRate.low,
