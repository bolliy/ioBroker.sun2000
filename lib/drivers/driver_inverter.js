...
class InverterInfo extends DriverBase {
		...
		const newFields = [
			{
				...
				readErrorHook: () => { //err,reg
					...
					if (!isSunshine(this.adapter)) this._errorCount ++;
					return true;
				},
				postHook: (path) => {
	...
	get modbusAllowed () {
		if (isSunshine(this.adapter)) {
			...
			if (this._errorCount > 2 && this._modbusAllowed) {
...
class InverterSun2000 extends DriverBase{
	constructor(stateInstance,inverter,options) {
		...
		//Test
		this._testMode = this.adapter.settings.address === '192.168.2.90';
	...
	get modbusAllowed () {
		...
		if (modbusAllowed) modbusAllowed = this.deviceStatus !== 0x0002;
		if (this._errorCount > 2) modbusAllowed = false;

		if (!modbusAllowed && !this.log.quiet) {
			this.log.info(`The inverter ${this.deviceInfo.index} is no longer accessible. That is why the logs are minimized.`);
			this.log.beQuiet(true);
		}
		if (modbusAllowed && this.log.quiet) {
			this.log.beQuiet(false);
			this.log.info(`The inverter ${this.deviceInfo.index} is accessible again.`);
			this._errorCount = 0;
		}
		this.log.beQuiet(!modbusAllowed);
