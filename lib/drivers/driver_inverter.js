		...
		const newHooks = [
			...
			},
			{
				...
				fn: path => {
					...
					let inputYield = this.stateCache.get(`${path}derived.dailyActiveEnergy`)?.value ?? 0;
					inputYield = inputYield * 0.97 + charge - disCharge;
					if (inputYield < 0) {
						inputYield = 0;
					}
			...
			},
			{
				...
				state: {
					...
					unit: 'kWh',
					role: 'value.power.consumption',
					desc: 'Riemann sum of sum of active power',
				},
				fn: path => {
