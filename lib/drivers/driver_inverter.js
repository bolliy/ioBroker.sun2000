...
const { alarmLevel, inverterAlarms1, inverterAlarms2, inverterAlarms3 } = require(`${__dirname}/../alarms.js`);
	...
	constructor(stateInstance, inverter, options) {
		...
		const newFields = [
			...
			},
			{
				...
				postHook: path => {
					//this.adapter.log.debug('postHook '+path);
					const alarm1 = this.stateCache.get(`${path}alarm1`)?.value;
					const alarm2 = this.stateCache.get(`${path}alarm2`)?.value;
					const alarm3 = this.stateCache.get(`${path}alarm3`)?.value;
					...
					if (alarm1 && alarm2 && alarm3) {
						...
						const alarm1Texts = this.textsFromBitfield(alarm1String, inverterAlarms1);
						const alarm2Texts = this.textsFromBitfield(alarm2String, inverterAlarms2);
						...
						this.adapter.log.silly(`Alarms from '${alarms}'`);
						//is.stateCache.set(`${path}derived.alarmsJSON`, getDeviceStatusInfo(deviceStatus));
					}

					//this.adapter.log.debug('postHook '+path+' done');
				},
			},
			{
				address: 32016,
