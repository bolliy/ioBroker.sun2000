...
class InverterSun2000 extends DriverBase {
	constructor(stateInstance, inverter, options) {
		...
		this.adapter.getState(`${this.deviceInfo.path}.derived.currentDayChargeCapacity`, (err, state) => {
			if (!err && state) {
				this.chargeSumHelper.setStart(state?.val, state?.ts);
			}
		});
		this.dischargeSumHelper = new RiemannSum();
		this.adapter.getState(`${this.deviceInfo.path}.derived.currentDayDischargeCapacity`, (err, state) => {
			if (!err && state) {
		...
		const newFields = [
			...
			},
			{
				...
				postHook: path => {
					...
					this.adapter.log.info(`### DEBUG1: Charge capacity from riemann sum: ${this.chargeSumHelper.sum} kW`);
					this.adapter.log.info(`### DEBUG1: Discharge capacity from riemann sum: ${this.dischargeSumHelper.sum} kW`);
			...
			*/
			...
			},
			{
				...
				postHook: path => {
					...
					this.adapter.log.info(`### DEBUG2: Current day charge capacity from inverter: ${currentDayChargeCapacity} kWh`);
					this.adapter.log.info(`### DEBUG2: Current day discharge capacity from inverter: ${currentDayDischargeCapacity} kWh`);
				},
			},
			{
				address: 47000,
