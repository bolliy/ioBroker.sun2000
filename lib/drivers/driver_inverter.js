...
class InverterSun2000 extends DriverBase{
		...
		const newFields = [
			...
			},
			{
				...
				states : [{
					...
					mapper: value => Promise.resolve(this._checkValidNumber(value,-1000,1000)),
			...
			},
			{
				...
				},
				{
					...
					mapper: value => Promise.resolve(this._checkValidNumber(value,-100000,100000))
				},
				{
	...
	_checkValidNumber( value, from = 0, until = 100, substWith = 0) {
		if (typeof value == 'number') {
			...
			this.log.error('_checkValidNumber '+value+' from: '+from+' until: '+until);
			return substWith;
		}
		return value;
	}
