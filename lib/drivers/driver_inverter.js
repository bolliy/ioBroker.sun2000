...
class InverterSun2000 extends DriverBase {
	constructor(stateInstance, inverter, options) {
		...
		this.adapter.getState(`${this.deviceInfo.path}.battery.currentDayChargeCapacity`, (err, state) => {
			if (!err && state) {
				this.chargeSumHelper.setStart(state?.val, state?.ts);
			}
		});
		...
		this.adapter.getState(`${this.deviceInfo.path}.battery.currentDayDischargeCapacity`, (err, state) => {
			if (!err && state) {
				this.dischargeSumHelper.setStart(state?.val, state?.ts);
			}
		});
		...
		const newFields = [
			...
			},
			{
				...
				postHook: path => {
					...
					this.stateCache.set(`${path}battery.currentDayChargeCapacity`, this.chargeSumHelper.sum, { type: 'number' });
					this.stateCache.set(`${path}battery.currentDayDischargeCapacity`, this.dischargeSumHelper.sum, { type: 'number' });
					this.adapter.log.debug(`### DEBUG1: Charge capacity from riemann sum: ${this.chargeSumHelper.sum} kW`);
					this.adapter.log.debug(`### DEBUG1: Discharge capacity from riemann sum: ${this.dischargeSumHelper.sum} kW`);
			...
			*/
			...
			},
			{
				...
				postHook: path => {
					...
					//adjust battery total charge and discharge with current value from inverter
					const currentDayChargeCapacity = this.stateCache.get(`${path}battery.currentDayChargeCapacity`)?.value ?? 0;
					this.stateCache.set(`${path}battery.currentDayChargeCapacity`, this.chargeSumHelper.sum, { type: 'number' });
					this.stateCache.set(`${path}battery.currentDayDischargeCapacity`, this.dischargeSumHelper.sum, { type: 'number' });
					if (currentDayChargeCapacity > this.chargeSumHelper.sum) {
						this.chargeSumHelper.setStart(currentDayChargeCapacity);
						this.stateCache.set(`${path}battery.currentDayChargeCapacity`, currentDayChargeCapacity, { type: 'number', renew: true });
						this.adapter.log.warn(
							`Set charge sum helper ${this.chargeSumHelper.sum} to current day charge capacity from inverter: ${currentDayChargeCapacity} kWh`,
						);
					}
					if (Math.abs(currentDayChargeCapacity - this.chargeSumHelper.sum) > 1) {
						this.adapter.log.warn(`Charge capacity difference from inverter: ${Math.abs(currentDayChargeCapacity - this.chargeSumHelper.sum)} kWh`);
					}
					const currentDayDischargeCapacity = this.stateCache.get(`${path}battery.currentDayDischargeCapacity`)?.value ?? 0;
					if (currentDayDischargeCapacity > this.dischargeSumHelper.sum) {
						this.dischargeSumHelper.setStart(currentDayDischargeCapacity);
						this.stateCache.set(`${path}battery.currentDayDischargeCapacity`, currentDayDischargeCapacity, { type: 'number', renew: true });
						this.adapter.log.warn(
							`Set discharge sum helper ${this.dischargeSumHelper.sum} to current day discharge capacity from inverter: ${currentDayDischargeCapacity} kWh`,
						);
					}
					if (Math.abs(currentDayDischargeCapacity - this.dischargeSumHelper.sum) > 1) {
						this.adapter.log.warn(
							`Discharge capacity difference from inverter: ${Math.abs(currentDayDischargeCapacity - this.dischargeSumHelper.sum)} kWh`,
						);
					}
					this.adapter.log.debug(`### DEBUG2: Current day charge capacity from inverter: ${currentDayChargeCapacity} kWh`);
					this.adapter.log.debug(`### DEBUG2: Current day discharge capacity from inverter: ${currentDayDischargeCapacity} kWh`);
				},
			},
			{
				address: 47000,
