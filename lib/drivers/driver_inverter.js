	...
	constructor(stateInstance,inverter,options) {
		...
		const newFields = [
			...
			},
			{
				...
				//checkIfActive: (path) => this.stateCache.get(path+'battery.unit.1.SN').value
				checkIfActive: (path) => {
					this.log.debug('#### isBatteryUnitExits'+this._isBatteryUnitExist(path,0));
					return this._isBatteryUnitExist(path,0);
				}
			},
			{
				address : 32064,
			...
			},
			{
				...
				checkIfActive: (path) => this._isBatteryUnitExist(path,0)
			},
			{   //for NRGKick
				address : 47000,
				length : 1,
		...
		const newHooks = [
			{
				...
				fn : (path) => {
					...
					if (state && state?.value !== '') {
						this.deviceInfo.numberBatteryUnits = 1;
						this.deviceInfo.battery.unit1.exist = true;
					}
					state = this.stateCache.get(path+'battery.unit.2.SN');
					if (state && state?.value !== '') {
						this.deviceInfo.numberBatteryUnits = 2;
						this.deviceInfo.battery.unit2.exist = true;
					}
					*/
				}
			},
	...
	constructor(stateInstance,inverter,options) {
		...
		const newFields = [
			...
			},
			{
				...
				checkIfActive: (path) => this._isBatteryUnitExist(path,2)
			},
			{   //for NRGKick
				address : 47089,
				length : 1,
