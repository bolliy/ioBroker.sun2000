		...
		const newFields = [
			...
			},
			{
				...
				postHook: path => {
					...
					const currentDayChargeCapacity = this.stateCache.get(`${path}battery.statistics.currentDayChargeCapacity`)?.value ?? 0;
					if (currentDayChargeCapacity - this.chargeSumHelper.sum > 0.09) {
					...
					const currentDayDischargeCapacity = this.stateCache.get(`${path}battery.statistics.currentDayDischargeCapacity`)?.value ?? 0;
					if (currentDayDischargeCapacity - this.dischargeSumHelper.sum > 0.09) {
						this.adapter.log.info(
					...
					this.stateCache.set(`${path}battery.statistics.currentDayChargeEnergy`, this.chargeSumHelper.sum, { type: 'number', stored: true });
					this.stateCache.set(`${path}battery.statistics.currentDayDischargeEnergy`, this.dischargeSumHelper.sum, { type: 'number', stored: true });
				},
			},
			{
				address: 32064,
