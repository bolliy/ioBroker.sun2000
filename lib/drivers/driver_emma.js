			...
			*/
			...
			},
			{
				...
				postHook: path => {
					...
					this.log.debug(`Found number of chargers ${numberOfChargers}`);
		...
		const newHooks = [
			{
				...
				refresh: dataRefreshRate.high,
				fn: path => {
					this.log.debug(`postUpdateHooks ${path} `);
				},
			},
		];
		...
		const newFields = [
			{
				address: 30000,
				length: 105,
				info: 'Emma Scharger sampled data',
				refresh: dataRefreshRate.high,
				states: [
					{
						state: { id: 'charger.offeringName', name: 'Offering name', type: 'string', role: 'value', desc: 'reg:30000, len:15' },
						register: { reg: 30000, type: dataType.string, length: 29 },
					},
					{
						state: { id: 'charger.esn', name: 'ESN', type: 'string', role: 'value', desc: 'reg:30015, len:16' },
						register: { reg: 30015, type: dataType.string, length: 31 },
					},
					{
						state: { id: 'charger.softwareVersion', name: 'Software version', type: 'string', role: 'value', desc: 'reg:30031, len:16' },
						register: { reg: 30031, type: dataType.string, length: 31 },
					},
					{
						state: {
							id: 'charger.ratedPower',
							name: 'Rated power',
							type: 'number',
							unit: 'kW',
							role: 'value.power',
