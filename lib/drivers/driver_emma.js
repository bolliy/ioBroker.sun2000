		...
		const newFields = [
			...
			*/
			...
			},
			{
				...
				states: [
					{
						state: { id: 'systemTime', name: 'System time', type: 'number', role: 'value', desc: 'reg:40470, len:2' },
						register: { reg: 40470, type: dataType.uint32 },
					},
					{
						state: { id: 'derived.systemTime', name: 'System time', type: 'number', unit: '', role: 'value.time', desc: 'fixed system time' },
					},
				],
				postHook: path => {
					...
					const time = this.stateCache.get(`${path}systemTime`)?.value;
					this.stateCache.set(`${path}derived.systemTime`, fixTime(time), { type: 'number' });
				},
			},
		];
