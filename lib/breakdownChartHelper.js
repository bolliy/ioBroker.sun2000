 ...
 *   Map of targetPath → array of numeric values aligned with the xAxis.
 * @returns {Array} eCharts series array (may be empty when entries is empty)
 */
function buildBreakdownSeries(entries, seriesData) {
	...
	return entries.map((bd, idx) => {
		...
		return {
			name: bd.name,
			type: 'bar',
			stack: 'consumptionBreakdown', // stacks all breakdown bars together
			xAxisIndex: 1,
			yAxisIndex: 2,
